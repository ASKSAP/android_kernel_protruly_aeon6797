...
static int tpd_proximity_value=1;
...
static const struct i2c_device_id ft5x0x_tpd_id[] = {{"ft8716_lead", 0}, {} };
static const struct of_device_id ft5x0x_dt_match[] = {
	{.compatible = "mediatek,gt_touch"},
	{},
};
	...
	case 5:
		{
			...
			//ipi_pkt.param.tcs.io_int = 10;//tpd_int_gpio_number;
			//ipi_pkt.param.tcs.io_rst = 62;//tpd_rst_gpio_number;
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
...
#ifdef FTS_CTL_FACE_DETECT
	u8 is_face_detect_enable, ps_data;
	s32 ret;
#endif
	char writebuf[63]={0};
	u8 fwversion = 0;
	...
	fts_read_reg(i2c_client, 0x02, &point_count);
	tpd_firmware_version[0] = fwversion; //sanford.lin
	printk("FW version=%x]\n", fwversion);
	printk("focal point_count=%d\n", point_count);
#if 0
	...
	printk("Number of touch points = %d\n", cinfo->count);
	...
	if(face_detect_enable==1)
	{
		...
		ret = i2c_smbus_read_i2c_block_data(i2c_client, PSENSOR_ENABLE_REG, 1, &is_face_detect_enable);
		if(ret < 0)
		{
		...
		if (FT_FACE_DETECT_ENABLE == is_face_detect_enable) {
			if (FT_FACE_DETECT_ON == ps_data )
			{
				tpd_proximity_value=0;
			}
			else if (FT_FACE_DETECT_OFF == ps_data)
			{
				tpd_proximity_value=1;
			}
		}
		
		TPD_DMESG("face_detect_enable:%d, tpd_proximity_value:%d\n", 
			face_detect_enable, tpd_proximity_value); 
...
int fts_i2c_write(struct i2c_client *client, char *writebuf, int writelen)
{
	...
	else if((writelen > 8)&&(0 != g_dma_buff_pa))
	{
		...
		//	g_dma_buff_pa[i] = (dma_addr_t)writebuf[i];
		memcpy(g_dma_buff_va,writebuf, writelen);
		client->addr = (client->addr & I2C_MASK_FLAG )| I2C_DMA_FLAG;
	    //client->ext_flag = client->ext_flag | I2C_DMA_FLAG;
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		printk("touch_event_handler start\n");
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (RECOVERY_BOOT == get_boot_mode() || KERNEL_POWER_OFF_CHARGING_BOOT == get_boot_mode() || LOW_POWER_OFF_CHARGING_BOOT == get_boot_mode() || META_BOOT == get_boot_mode())   // important
	{
		tpd_load_status = 0;
		return -1;
	}
    ...
    if ((i2c_smbus_read_i2c_block_data(i2c_client, 0xA3, 1, &data)) < 0) {
		...
		TPD_DMESG("I2C transfer,data=0x%x, line: %d\n", data, __LINE__);
    }
#endif
	/* set INT mode */
...
static int tpd_local_init(void)
{
	...
	tpd_dts_data.tpd_key_num = TPD_KEY_COUNT;
...
{
	TPD_DMESG("%s:tpd_proximity_value = %d\n", __func__, tpd_proximity_value);
	return tpd_proximity_value;
}
 
static int tp_ps_open_report_data(int open)
{
	//should queuq work to report event if  is_report_input_direct=true
	return 0;
}
 
static int tp_ps_enable_nodata(int value)
{
	TPD_DMESG("%s:value = %d\n", __func__ ,value);
	if(value)
	{
...
static int tp_ps_set_delay(u64 ns)
{
	 return 0;
}
 
static int tp_ps_get_data(int* value, int* status)
{
...
static int  tpd_ps_local_init(void)
{
	int err=0;
	struct ps_control_path ps_ctl={0};
	struct ps_data_path ps_data={0};

	//if (ft_psensor_drv_init(i2c_client) < 0)
	//	pr_err("%s:[FTS] create fts control psensor driver failed\n",__func__);

	ps_ctl.open_report_data= tp_ps_open_report_data;
	ps_ctl.enable_nodata = tp_ps_enable_nodata;
	ps_ctl.set_delay  = tp_ps_set_delay;
	...
	if(err)
	{
		pr_err("ps_register_control_path fail = %d\n", err);
		goto als_ps_register_err;
	}
	ps_data.get_data = tp_ps_get_data;
	ps_data.vender_div = 1;
	err = ps_register_data_path(&ps_data);	
	if(err)
	{
		pr_err("ps_register_data_path fail = %d\n", err);
		goto als_ps_register_err;
	}
	...
	als_ps_register_err:
	pr_err("als_ps_register_err\n");
	return -1;

}

static int tpd_ps_remove(void)
{
	TPD_DMESG("tpd_ps remove !\n");
	return 0;
}

static struct alsps_init_info tpd_ps_init_info = {			
	.name = "tpd_ps",			
	.init = tpd_ps_local_init,			
	.uninit =tpd_ps_remove,	
};
#endif
...
#ifdef FTS_CTL_FACE_DETECT		
 	if(face_detect_enable && !tp_suspend_flag)  return;
#endif
...
#ifdef FTS_CTL_FACE_DETECT
	if(alsps_driver_add(&tpd_ps_init_info)<0)
		TPD_DMESG("add tpd_ps driver failed\n");
#endif
