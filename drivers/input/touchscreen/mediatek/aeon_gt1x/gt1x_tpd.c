...
#else
			GTP_ERROR("##########unable CONFIG_ARCH_MT6580\n");
...
static int tpd_irq_registration(void)
{
	...
	if (node) {
		...
		GTP_INFO("Device gt1x_int_type = %d!,>>>>>touch_irq = %d", gt1x_int_type,touch_irq);
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	GTP_INFO("gt1x tpd_i2c_probe start.");
...
#endif
	GTP_INFO("gt1x tpd_i2c_probe end......");
	tpd_load_status = 1;
	return 0;
}
...
static irqreturn_t tpd_eint_interrupt_handler(unsigned irq, struct irq_desc *desc)
{
	...
	TPD_DEBUG_PRINT_INT;
	tpd_flag = 1;
	//GTP_INFO("gtp tpd_eint_interrupt_handler......");
	spin_lock_irqsave(&irq_flag_lock, flags);
	if (irq_flag == 0) {
		spin_unlock_irqrestore(&irq_flag_lock, flags);
		return IRQ_HANDLED;
