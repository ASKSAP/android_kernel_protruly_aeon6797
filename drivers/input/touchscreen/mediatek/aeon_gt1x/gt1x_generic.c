...
s32 gt1x_send_cfg(u8 *config, int cfg_len)
{
	...
	GTP_ERROR("Driver Send Config, length: %d", cfg_len);
	...
	checksum = 0 - checksum;
	GTP_ERROR("Config checksum: 0x%04X", checksum);
...
s32 gt1x_init_panel(void)
{
	...
	GTP_ERROR("Config Groups Length: %d, %d, %d, %d, %d, %d", cfg_lens[0], cfg_lens[1], cfg_lens[2], cfg_lens[3],
	...
	ret2 = gt1x_i2c_read_dbl_check(GTP_REG_SENSOR_ID, id_buf, sizeof(id_buf));
	if (!ret2) {
		if(id_buf[0]==1 || id_buf[1]==0x1b)   //for 70pin
		{      
			GTP_ERROR("cfg_grp0_new 70 pin\n"); 
			cfgs[0] = cfg_grp0_new;
		}else{
			GTP_ERROR("cfg_grp0_new 50 pin\n"); 
		}
		/*
		else if(id_buf[0]==0 || id_buf[1]==0x1c)    //for 50pin
		{
			cfgs[1] = cfg_grp0;
		}
		 */		
		GTP_ERROR("GTP_REG_SENSOR_ID =%x-%x-%x-%x\n",id_buf[0],id_buf[1],id_buf[2],id_buf[3]); 
	} else {
	...
	GTP_ERROR("Res: %d * %d, trigger: %d", tpd_dts_data.tpd_resolution[0],
	...
	GTP_ERROR("X_MAX=%d,Y_MAX=%d,TRIGGER=0x%02x,WAKEUP_LEVEL=%d", gt1x_abs_x_max, gt1x_abs_y_max, gt1x_int_type,
		 gt1x_wakeup_level);
...
s32 gt1x_read_version(struct gt1x_version_info *ver_info)
{
	...
	while (retry--) {
		...
		GTP_ERROR("GTP reread version : %d", retry);
		msleep(100);
	}
