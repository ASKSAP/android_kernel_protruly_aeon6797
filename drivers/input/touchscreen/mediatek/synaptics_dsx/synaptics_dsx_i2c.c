...
#include "tpd_synaptics_common.h"
...
static ssize_t synaptics_rmi4_NS_Calibration_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);		
...
static struct device_attribute attrs[] = {
	...
	__ATTR(ns_calibrate, 0660,
			synaptics_rmi4_show_error,
			synaptics_rmi4_NS_Calibration_store),
};
...
#define F54_ANALOG_CTRL188 0x012e			//NS Calibration 
static ssize_t synaptics_rmi4_NS_Calibration_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int retval;
	unsigned short ns_cali_addr;
	unsigned char ns_cali_data;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	
	ns_cali_addr = F54_ANALOG_CTRL188;
	ns_cali_data = 0x01;
	...
	if (retval < 0) {
		dev_err(dev,
				"%s: Failed to write NS Calibrate Reg , error = %d\n",
				__func__, retval);
		return retval;
	}
	...
	if (ns_cali_data != 0) {
		dev_err(dev,
				"%s: Failed to do NS Calibration , error = %d\n",
				__func__, retval);
		return retval;
	}
	
	printk("Do NS Calibration Successful !!\n");
	return count;
}
...
static void tpd_down(struct synaptics_rmi4_data *rmi4_data, int x, int y)
{
	...
	input_report_abs(rmi4_data->input_dev, ABS_MT_POSITION_X, x*rmi4_data->sensor_max_x/SENSOR_MAX_X);
	input_report_abs(rmi4_data->input_dev, ABS_MT_POSITION_Y, y*rmi4_data->sensor_max_y/SENSOR_MAX_Y);
		...
		unsigned int intr_count)
{
	...
	/* Maximum x and y */
	rmi4_data->sensor_max_x = SENSOR_MAX_X; //control_6_9.sensor_max_x_pos_7_0 | (control_6_9.sensor_max_x_pos_11_8 << 8);
	rmi4_data->sensor_max_y = SENSOR_MAX_Y; //control_6_9.sensor_max_y_pos_7_0 | (control_6_9.sensor_max_y_pos_11_8 << 8);
...
{
	printk("synaptics I2C Touchscreen Driver\n");

	if(i2c_add_driver(&tpd_i2c_driver)!=0)
	{
		pr_err("Synaptics Error unable to add i2c driver.\n");
		return -ENODEV;
	}
	if(tpd_load_status == 0)
	{
		pr_err("Synaptics add error touch panel driver.\n");
