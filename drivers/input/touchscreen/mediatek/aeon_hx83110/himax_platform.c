...
static int himax_tpd_int_gpio = 85;
...
int himax_parse_dt(struct himax_ts_data *ts,struct himax_i2c_platform_data *pdata)
{
	...
	if (dt) {
		...
		if (!match) {
			printk("[Himax]Error: No device match found\n");
			return -ENODEV;
		}
	}
...
#endif
#else
	printk("himax irq enable=%d , irq_enable_count=%d\n",enable,irq_enable_count);
...
int himax_ts_register_interrupt(struct i2c_client *client)
{
	...
	if(ic_data->HX_INT_IS_EDGE)
		{
			printk("himax edge triiger falling\n ");
...
static int himax_common_local_init(void)
{
	...
	//{
	//	printk("add error hx83110 touch panel driver.\n");
