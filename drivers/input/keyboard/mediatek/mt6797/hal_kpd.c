...
//#ifdef CONFIG_MTK_HOMEPAGE_KEY
static int homepage_eint_state;
static int homepage_ext_rst_irq;
static irqreturn_t homepage_rst_eint_handler(int irq, void *data)
{
	/* bool pressed; */
	printk("of_find_matching_node homepage_eint_state=%d\n",homepage_eint_state);
	if (homepage_eint_state == 0) {
		irq_set_irq_type(homepage_ext_rst_irq, IRQ_TYPE_LEVEL_HIGH);
		homepage_eint_state = 1;
	} else {
		irq_set_irq_type(homepage_ext_rst_irq, IRQ_TYPE_LEVEL_LOW);
		homepage_eint_state = 0;
	}

	input_report_key(kpd_input_dev, KEY_HOME, homepage_eint_state);
	input_sync(kpd_input_dev);
...
//#ifdef CONFIG_MTK_HOMEPAGE_KEY
void mt_homepage_eint_register(void)
{
	int ints[2] = {0, 0};
	int ret;
	struct device_node *node;
	...
	/* register EINT handler for MRDUMP_EXT_RST key */
	node = of_find_compatible_node(NULL, NULL,"mediatek, homepage-eint");
	if (!node)
		printk("mt_homepage_eint_register can't find homepage compatible node\n");
	else {
		of_property_read_u32_array(node, "debounce", ints, ARRAY_SIZE(ints));
		gpio_set_debounce(ints[0], ints[1]);
        printk("mt_homepage_eint_register ints[0]=%d,ints[1]=%d\n",ints[0],ints[1]);
		homepage_ext_rst_irq = irq_of_parse_and_map(node, 0);
		ret = request_irq(homepage_ext_rst_irq, homepage_rst_eint_handler,
				  IRQF_TRIGGER_NONE, "homepage-eint", NULL);
		if (ret > 0)
			printk("mt_homepage_eint_register EINT IRQ LINE NOT AVAILABLE\n");
	}
