...
//static int ta_recheck_count = 0;
kal_bool ta_recheck_flag = KAL_FALSE;
/**************end***************/
...
static void mtk_ta_detector(void)
{
	...
	if ((real_v_chrB - ta_v_chr_org >= 1000) && (g_call_state == 0))
...
{
	battery_log(BAT_LOG_CRTI,
			    "[PE+]start battery_pump_express_charger_check, SOC=%d, ta_check_chr_type = %d, charger_type = %d ,g_call_state=%d\n",
			    BMT_status.SOC, ta_check_chr_type, BMT_status.charger_type,g_call_state);
...
#define CUSTOM_CHARGER_SWITCH_V1 1
...
/*For BQ25896, voltage is used to check if PE+ should be tuned off.*/
		...
		if(g_call_state == 1)
		{
			...
			//battery_log(BAT_LOG_CRTI,
			//	    "[PE+]calling active. Stopping PE+, is_ta_connect =%d\n",is_ta_connect);			
		}
		
		/***********zhaolong add for recheck PE+ start...***********/
		if (abs(charger_vol - ta_v_chr_org) < 1000 && BMT_status.bat_charging_state == CHR_CC && ta_check_chr_type == KAL_FALSE) {
				ta_recheck_flag = KAL_TRUE;
				battery_log(BAT_LOG_CRTI, "[PE+] abnormal TA chager voltage, rechecking PE+ adapter\n");
			}
		/**************end******************/
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if(charging_enable == KAL_TRUE){
			...
			if(limit>0){
				...
				if(limit == 1){
					if ((is_ta_connect == KAL_TRUE)||(charger_vol>6000)){
					...
					if(g_call_state == 1)
					{
						MAX_CURRENT = CHARGE_CURRENT_1000_00_MA;
					}
					else
					{	
						if ((is_ta_connect == KAL_TRUE)||(charger_vol>6000)){
...
/**********zhaolong add***********************/
void battery_pump_express_charger_recheck(void)
{
	if(g_call_state == 0)
	{	
			ta_check_chr_type = KAL_TRUE;
			ta_recheck_flag = KAL_FALSE;
			printk("zhaolong debug PE+:try to recheck PE+\n");
	}
}
/******************end**************************/
...
PMU_STATUS BAT_ConstantCurrentModeAction(void)
{
	...
	/**********zhaolong add**********************/
	printk("zhaolong debug PE+:ta_recheck_flag=%d\n", ta_recheck_flag);
	if (ta_recheck_flag == KAL_TRUE)				
		battery_pump_express_charger_recheck();
	/******************end****************************/
