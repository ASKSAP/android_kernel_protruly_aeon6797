...
#if defined(CONFIG_MTK_OZ1C105C_SUPPORT)
...
#endif
#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
		battery_log(BAT_LOG_CRTI, "zhaolong:%s[BATTERY] BMT_status.charger_vol=%d ,batt_cust_data.v_charger_max=%d mV\n",	
				__func__, BMT_status.charger_vol, batt_cust_data.v_charger_max);
...
static void mt_battery_notify_VBat_check(void)
{
	...
	if (BMT_status.bat_vol > 4500)
		/* if (BMT_status.bat_vol > 3800) //test */
	{
		g_BatteryNotifyCode |= 0x0008;
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_vlot(%d) > 4500mV\n", BMT_status.bat_vol);
	} else {
		g_BatteryNotifyCode &= ~(0x0008);
	}
...
static void mt_battery_notify_VCharger_check(void)
{
	...
	battery_log(BAT_LOG_CRTI, "zhaolong:%s BMT_status.charger_vol=%d, batt_cust_data.v_charger_max=%d mV\n",
			    __func__, BMT_status.charger_vol, batt_cust_data.v_charger_max);
	if (BMT_status.charger_vol > batt_cust_data.v_charger_max) {
#else
	if (BMT_status.charger_vol > v_charger_max) {
...
//extern int mt_brightness_set_pmic_combo(enum mt65xx_led_pmic pmic_type, u32 level, u32 div);
#ifdef CONFIG_MTK_BQ25896_SUPPORT
...
void BAT_thread(void)
{
    ...
    battery_log(BAT_LOG_CRTI, "BAT_thread  .\n");
...
int bat_routine_thread(void *x)
{
	...
	while (1) {
		...
		if ((((chargin_hw_init_done == KAL_TRUE) && (battery_suspended == KAL_FALSE)) || ((chargin_hw_init_done == KAL_TRUE) && (chr_wake_up_bat == KAL_TRUE))) || (((chargin_hw_init_done_rt == KAL_TRUE) && (battery_suspended == KAL_FALSE)) || ((chargin_hw_init_done_rt == KAL_TRUE) && (chr_wake_up_bat == KAL_TRUE))))
			BAT_thread();
...
#ifdef CONFIG_MTK_BQ25896_SUPPORT
extern unsigned int bq25890_get_wdt_state(void);
...
int charger_hv_detect_sw_thread_handler(void *unused)
{
	...
	do {
		...
		#ifdef CONFIG_MTK_BQ25896_SUPPORT
		to=bq25890_get_wdt_state();
		battery_log(BAT_LOG_CRTI,"charger_hv_detect_sw_thread_handler to=%d\n",to);
		if(to == 1)
		{
