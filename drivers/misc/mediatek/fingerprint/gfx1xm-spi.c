...
static unsigned long long local_buf_size;
...
static int spi_local_buff_map(void)
{
...
static void gfx1xm_spi_set_mode(struct spi_device *spi, SPI_SPEED speed, int flag)
{
	...
	printk("gfx1xm_spi_set_mode flag=%d\n\n",flag);
	if(flag == 0) {
				...
				u16 addr, u32 data_len, u8 *tx_buf)
{
	...
	int use_dma_buff = 0;
#endif
	//printk("gfx1xm_spi_write_bytes data_len + GFX1XM_WDATA_OFFSET=%d\n",data_len + GFX1XM_WDATA_OFFSET);
	...
	if((package_num > 0) && (reminder != 0)) {
        ...
        printk("gfx1xm:w-reminder:0x%x-0x%x,0x%x", reminder_buf[GFX1XM_WDATA_OFFSET],reminder_buf[GFX1XM_WDATA_OFFSET+1],
				...
				u16 addr, u32 data_len, u8 *rx_buf)
{
	...
	int use_dma_buff = 0;
#endif	
	//printk("gfx1xm_spi_read_bytes data_len + GFX1XM_RDATA_OFFSET=%d\n",data_len + GFX1XM_RDATA_OFFSET);
	/*set spi mode.*/
	...
	if((package_num > 0) && (reminder != 0)) {
		twice = 1;
		//printk("gfx1xm_spi_read_bytes USE_SPI1_4GB 1 \n");
#if USE_SPI1_4GB 
		if(1 == use_dma_buff)
	...
	if(twice == 1) {
		...
		xfer[2].len = 3;
#if USE_SPI1_4GB
        //printk("gfx1xm_spi_read_bytes USE_SPI1_4GB 4 \n");
		if(1 == use_dma_buff){
			memcpy_toio(spi_tx_local_buf+xfer[1].len, reminder_buf, xfer[2].len);
			xfer[2].tx_buf = spi_tx_local_buf + xfer[1].len;
			xfer[2].tx_dma = SpiDmaBufTx_pa + xfer[1].len;
		}
#endif
...
#if USE_SPI1_4GB
        //printk("gfx1xm_spi_read_bytes USE_SPI1_4GB 5 \n");
        if(1 == use_dma_buff){
            memset_io(spi_tx_local_buf, 0, xfer[3].len);
            *spi_tx_local_buf = reminder_buf[4];
            xfer[3].tx_buf = spi_tx_local_buf;
            xfer[3].tx_dma = SpiDmaBufTx_pa;
            xfer[3].rx_dma = SpiDmaBufRx_pa + (&reminder_buf[4]-(u8 *)spi_rx_local_buf);//This line is modified.
        }else
#endif
                {
...
#if USE_SPI1_4GB
			if(1 == use_dma_buff)
				memcpy_fromio(rx_buf + GFX1XM_RDATA_OFFSET + data_len - reminder + 1, reminder_buf + GFX1XM_RDATA_OFFSET, reminder);
			else
#endif
...
#error "have't define any product for fw"
	...
	//xielei @20160108
	//finger_irq_node= of_find_compatible_node(NULL,NULL, "mediatek, finger");
