...
struct bmg_i2c_data {
	...
	s16					 inv_cali_raw[BMG_AXES_NUM+1];
	s16					 temperature;
...
static struct platform_device *gyroPltFmDev;
/* 0=OK, -1=fail */
...
#if INV_GYRO_AUTO_CALI == 1
/*----------------------------------------------------------------------------*/
static int BMI160_ReadGyroDataRaw(struct i2c_client *client, char *buf, int bufsize)
{
	struct bmg_i2c_data *obj = i2c_get_clientdata(client);

	mutex_lock(&obj->raw_data_mutex);
	/* return gyro raw LSB in device orientation */
	sprintf(buf, "%x %x %x", obj->inv_cali_raw[BMG_AXIS_X], obj->inv_cali_raw[BMG_AXIS_Y],
		obj->inv_cali_raw[BMG_AXIS_Z]);
...
/*----------------------------------------------------------------------------*/
static int BMI160_ReadTemperature(struct i2c_client *client, char *buf, int bufsize)
{
	struct bmg_i2c_data *obj = i2c_get_clientdata(client);
		...
		char *buf, int bufsize)
{
...
#if INV_GYRO_AUTO_CALI == 1
	ret = bmg_i2c_read_block(client, BMI160_OUT_TEMP_L, tmpbuf, 2);
	if(bmg_i2c_read_block(client, BMI160_OUT_TEMP_L, tmpbuf, 2))
	{
		GYRO_ERR("BMI160 read temperature data  error\n");
		return -2;
	}
	mutex_lock(&obj->temperature_mutex);
	obj->temperature = ((s16)((tmpbuf[1]) | (tmpbuf[0] << 8)));
	mutex_unlock(&obj->temperature_mutex);
#endif	
	...
	if (err) {
	  ...
	  {
			/* Data X */
			databuf[BMG_AXIS_X] = (s16)((((s32)((s8)buf_tmp[1])) << BMI160_SHIFT_8_POSITION) | (buf_tmp[0]));
			/* Data Y */
			databuf[BMG_AXIS_Y] = (s16)((((s32)((s8)buf_tmp[3])) << BMI160_SHIFT_8_POSITION) | (buf_tmp[2]));
			/* Data Z */
			databuf[BMG_AXIS_Z] = (s16)((((s32)((s8)buf_tmp[5])) << BMI160_SHIFT_8_POSITION) | (buf_tmp[4]));
		...
		mutex_lock(&obj->raw_data_mutex);
		/*remap coordinate*/
		obj->inv_cali_raw[obj->cvt.map[BMG_AXIS_X]] = obj->cvt.sign[BMG_AXIS_X]*databuf[BMG_AXIS_X];
		obj->inv_cali_raw[obj->cvt.map[BMG_AXIS_Y]] = obj->cvt.sign[BMG_AXIS_Y]*databuf[BMG_AXIS_Y];
		obj->inv_cali_raw[obj->cvt.map[BMG_AXIS_Z]] = obj->cvt.sign[BMG_AXIS_Z]*databuf[BMG_AXIS_Z];
		mutex_unlock(&obj->raw_data_mutex);
	#endif
		...
		gyro[BMG_AXIS_X] = (gyro[BMG_AXIS_X] * 131) / obj->sensitivity;
		gyro[BMG_AXIS_Y] = (gyro[BMG_AXIS_Y] * 131) / obj->sensitivity;
		gyro[BMG_AXIS_Z] = (gyro[BMG_AXIS_Z] * 131) / obj->sensitivity;
		...
		unsigned long arg)
{
	...
	case GYROSCOPE_IOCTL_SET_CALI:
		...
		if (atomic_read(&obj->suspend)) {
			...
			cali[BMG_AXIS_X] = sensor_data.x * obj->sensitivity/131;
			cali[BMG_AXIS_Y] = sensor_data.y * obj->sensitivity/131;
			cali[BMG_AXIS_Z] = sensor_data.z * obj->sensitivity/131;
	...
	case GYROSCOPE_IOCTL_GET_CALI:
		...
		sensor_data.x = (cali[BMG_AXIS_X]*131)/obj->sensitivity; 
		sensor_data.y = (cali[BMG_AXIS_Y]*131)/obj->sensitivity;
		sensor_data.z = (cali[BMG_AXIS_Z]*131)/obj->sensitivity;
		...
		case GYROSCOPE_IOCTL_READ_SENSORDATA_RAW:
			...
			BMI160_ReadGyroDataRaw(client, strbuf, BMG_BUFSIZE);
		...
		case GYROSCOPE_IOCTL_READ_TEMPERATURE:
			...
			BMI160_ReadTemperature(client, strbuf, BMG_BUFSIZE);
			if (copy_to_user(data, strbuf, sizeof(strbuf))) {
				err = -EFAULT;
				break;
			}
