...
static unsigned int charging_hw_init(void *data)
{
	...
	bq25890_config_interface(bq25890_CON7, 0x0, 0x3, 4);	/* enable  watch dog 40 secs 0x1 */
...
static unsigned int charging_set_current(void *data)
{
	...
	backlight_level = mt_get_bl_brightness();
	printk("zhaolong debug charging:backlight_level = %d\n" ,backlight_level);
	if (backlight_level > 0)
	{
		register_value = 32;	//2048mA
	}
	else
	{
		array_size = GETARRAYNUM(CS_VTH);
	...
	printk("zhaolong debug charging:register_value=%d\n" ,register_value);
