...
static int cam_cal_get_cmd_info(unsigned int sensorID, stCAM_CAL_CMD_INFO_STRUCT *cmdInfo, unsigned int u4Offset, unsigned int  ui4_length)
{
			...
			if (pCamCalList[i].sensorID == sensorID)
			{
						        ...
						        || pCamCalList[i].cmdType == CMD_AUTO)
						{
							if (pCamCalList[i].checkFunc != NULL)
							{
								if (pCamCalList[i].checkFunc(cmdInfo->client,
								                             pCamCalFunc[j].readCamCalData, u4Offset, ui4_length))
								{
				...
				else if (pCamCalList[i].cmdType == CMD_DEV1)
				{
					...
					if (cam_cal_check_double_eeprom(g_camCalDrvInfo[0].client,
					                                g_camCalDrvInfo[0].readCMDFunc, u4Offset, ui4_length))
					{
...
static stCAM_CAL_CMD_INFO_STRUCT *cam_cal_get_cmd_info_ex(unsigned int sensorID, unsigned int u4Offset, unsigned int  ui4_length)
{
	int i = 0;
	...
	if (i == CAM_CAL_I2C_MAX_SENSOR)
	{
		for (i = 0; i < CAM_CAL_I2C_MAX_SENSOR; i++)
		{
			if (g_camCalDrvInfo[i].sensorID == 0)
			{
				...
				cam_cal_get_cmd_info(sensorID, &g_camCalDrvInfo[i], u4Offset, ui4_length);
...
#endif
{
	...
	case CAM_CALIOC_S_WRITE:/*Note: Write Command is Unverified!*/
		...
		pcmdInf = cam_cal_get_cmd_info_ex(ptempbuf->sensorID, ptempbuf->u4Offset, ptempbuf->u4Length);
