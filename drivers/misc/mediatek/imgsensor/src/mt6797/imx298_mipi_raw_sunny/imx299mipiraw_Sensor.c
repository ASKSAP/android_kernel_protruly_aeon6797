...
#include "imx299mipiraw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "IMX299_camera_sensor"
#define LOG_1 LOG_INF("IMX299,MIPI 4LANE\n")
...
static BYTE imx299_SPC_data[252]={0};

extern void read_imx299_SPC( BYTE* data );
extern void read_imx299_DCC( kal_uint16 addr,BYTE* data, kal_uint32 size);

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX299_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#define IMX299MIPI_MaxGainIndex (115)
kal_uint16 IMX299MIPI_sensorGainMapping[IMX299MIPI_MaxGainIndex][2] ={	
	{64 , 1  },
	{65 ,8	},
	{66 ,16 },
	{67 ,25 },
...
#if 0
static kal_uint32 imx299_ATR(UINT16 DarkLimit, UINT16 OverExp)
{
/*
...
static void imx299_apply_SPC(void)
{
	...
	{
		read_imx299_SPC(imx299_SPC_data);
		read_spc_flag = TRUE;
		return;
	}
	...
	for(i=0;i<126;i++)
	{
		write_cmos_sensor(start_reg, imx299_SPC_data[i]);
		//LOG_INF("SPC[%d]= %x \n", i , imx299_SPC_data[i]);	
		start_reg++;
	}
	start_reg=0x7f00;
	for(j=0;j<126;j++)
	{
		write_cmos_sensor(start_reg, imx299_SPC_data[i+j]);
		//LOG_INF("SPC[%d]= %x \n", i , imx299_SPC_data[i]);
...
{
	kal_uint8 iI;
    LOG_INF("[IMX299MIPI]enter IMX299MIPIGain2Reg function\n");
    for (iI = 0; iI < IMX299MIPI_MaxGainIndex; iI++) 
	{
		if(gain < IMX299MIPI_sensorGainMapping[iI][0])
		{                
			return IMX299MIPI_sensorGainMapping[iI][1];       
		}
			

    }
	if(iI != IMX299MIPI_MaxGainIndex)
	{
    	if(gain != IMX299MIPI_sensorGainMapping[iI][0])
    	{
        	 LOG_INF("Gain mapping don't correctly:%d %d \n", gain, IMX299MIPI_sensorGainMapping[iI][0]);
    	}
    }
	LOG_INF("exit IMX299MIPIGain2Reg function\n");
    return IMX299MIPI_sensorGainMapping[iI-1][1];
}
...
static void preview_setting(void)
{
   ...
   //write_cmos_sensor(0x3104,0x01);//x_end
	write_cmos_sensor(0x3165,0x02); //0:16x12;1:8x6;2:free area
    write_cmos_sensor(0x3166,0x01);//flexible window mode 
	write_cmos_sensor(0x3110,0x03);
   /************PD 8*6 window*************/
	write_cmos_sensor(0x3111,0x68);
	write_cmos_sensor(0x3112,0x02); //542
	write_cmos_sensor(0x3113,0x46);
	write_cmos_sensor(0x3114,0x05);
	write_cmos_sensor(0x3115,0xB0);
	write_cmos_sensor(0x3116,0x04); //1205
	write_cmos_sensor(0x3117,0x8E);
...
static void capture_setting(kal_uint16 currefps)
{
   ...
   if(currefps==150)
   {
   ...
   else
	{
	   ...
	   write_cmos_sensor(0x3166,0x01);//flexible window mode 
	   /************flexible window*************/
                   write_cmos_sensor(0x3110,0x06);
                   write_cmos_sensor(0x3111,0xBB);
                   write_cmos_sensor(0x3112,0x04);
                   write_cmos_sensor(0x3113,0x77);
                   write_cmos_sensor(0x3114,0x0B);
                   write_cmos_sensor(0x3115,0x75);
                   write_cmos_sensor(0x3116,0x09);
                   write_cmos_sensor(0x3117,0x31);  
	   /************PD 8*6 window*************/
/*	   write_cmos_sensor(0x3108,0x00);//x_start
...
static kal_uint32 imx299_awb_gain(SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
	UINT32 rgain_32, grgain_32, gbgain_32, bgain_32;
    LOG_INF("imx299_awb_gain\n");
    ...
    LOG_INF("[imx299_awb_gain] ABS_GAIN_GR:%d, grgain_32:%d\n", pSetSensorAWB->ABS_GAIN_GR, grgain_32);
    LOG_INF("[imx299_awb_gain] ABS_GAIN_R:%d, rgain_32:%d\n", pSetSensorAWB->ABS_GAIN_R, rgain_32);
    LOG_INF("[imx299_awb_gain] ABS_GAIN_B:%d, bgain_32:%d\n", pSetSensorAWB->ABS_GAIN_B, bgain_32);
    LOG_INF("[imx299_awb_gain] ABS_GAIN_GB:%d, gbgain_32:%d\n", pSetSensorAWB->ABS_GAIN_GB, gbgain_32);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:	
			...
			read_imx299_DCC((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
            break;
...
UINT32 IMX299_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    IMX299_MIPI_RAW_SensorInit    */
