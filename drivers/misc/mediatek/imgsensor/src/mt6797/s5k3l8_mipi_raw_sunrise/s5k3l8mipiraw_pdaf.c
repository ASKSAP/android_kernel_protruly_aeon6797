...
#define DATA_SIZE         1404 //1404
#define START_ADDR        0x1801//0X0763
BYTE S5K3L8_eeprom_data_sunrise[DATA_SIZE]= {0};
...
static bool _read_eeprom(kal_uint16 addr, kal_uint32 size ){
  ...
  int i=0;
  for(; i<size; i++){
    S5K3L8_eeprom_data_sunrise[i] = read_cmos_sensor_byte(addr+i);
    //LOG_INF("add = 0x%x,\tvalue = 0x%x",i, S5K3L8_eeprom_data_sunrise[i]);
    printk("--->>>_read_eeprom_sunrise: add = 0x%x, value = 0x%x \n",i, S5K3L8_eeprom_data_sunrise[i]);
  }
  return true;
}

bool S5K3L8_read_eeprom_sunrise( kal_uint16 addr, BYTE* data, kal_uint32 size){
	addr = START_ADDR;
	size = DATA_SIZE;

	//LOG_INF("Read EEPROM, addr = 0x%x, size = 0d%d\n", addr, size);
	printk("--->>>S5K3L8_read_eeprom_sunrise: addr = 0x%x, size = 0d%d\n", addr, size);
	...
	memcpy(data, S5K3L8_eeprom_data_sunrise, size);
	return true;
}
