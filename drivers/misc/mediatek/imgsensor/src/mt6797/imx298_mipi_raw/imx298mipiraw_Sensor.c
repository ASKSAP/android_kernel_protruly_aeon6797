...
static void imx298_apply_SPC(void)
{
	...
	for(i=0;i<126;i++)
	{
		...
		//LOG_INF("write start_reg_L = 0x%x SPC[%d]= 0x%x \n", start_reg, i , imx298_SPC_data[i]);	
		//LOG_INF("read  start_reg_L = 0x%x value = 0x%x \n", start_reg, read_cmos_sensor(start_reg));	
		start_reg++;
	}
	start_reg=0x7f00;
	for(j=0;j<126;j++)
	{
		write_cmos_sensor(start_reg, imx298_SPC_data[i+j]);
		//LOG_INF("write start_reg_R = 0x%x SPC[%d]= %x \n", start_reg, i , imx298_SPC_data[i+j]);
		//LOG_INF("read  start_reg_R = 0x%x value = 0x%x \n", start_reg, read_cmos_sensor(start_reg));
