...
#define SUPPORT_I2C_BUS_NUM_MAIN        			SUPPORT_I2C_BUS_NUM1
#define SUPPORT_I2C_BUS_NUM_MAIN_CUSTOM		SUPPORT_I2C_BUS_NUM1
#define SUPPORT_I2C_BUS_NUM_MAIN2        		SUPPORT_I2C_BUS_NUM1
#define SUPPORT_I2C_BUS_NUM_SUB        			SUPPORT_I2C_BUS_NUM2
#define SUPPORT_I2C_BUS_NUM_SUB_SUB         		SUPPORT_I2C_BUS_NUM3
...
#ifdef MTK_MULT_CAMERA
BOOL g_bEnableDriver[KDIMGSENSOR_MAX_INVOKE_DRIVERS] = {FALSE, FALSE, FALSE, FALSE};
SENSOR_FUNCTION_STRUCT *g_pInvokeSensorFunc[KDIMGSENSOR_MAX_INVOKE_DRIVERS] = {NULL, NULL, NULL, NULL};
...
CAMERA_DUAL_CAMERA_SENSOR_ENUM g_invokeSocketIdx[KDIMGSENSOR_MAX_INVOKE_DRIVERS] = {DUAL_CAMERA_NONE_SENSOR, DUAL_CAMERA_NONE_SENSOR, DUAL_CAMERA_NONE_SENSOR, DUAL_CAMERA_NONE_SENSOR};
char g_invokeSensorNameStr[KDIMGSENSOR_MAX_INVOKE_DRIVERS][32] = {KDIMGSENSOR_NOSENSOR, KDIMGSENSOR_NOSENSOR, KDIMGSENSOR_NOSENSOR, KDIMGSENSOR_NOSENSOR};
#else
...
MINT32 i = 0;
    ...
    if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
        ...
        else if (DUAL_CAMERA_MAIN_3_SENSOR == g_invokeSocketIdx[i]) {
            spin_lock(&kdsensor_drv_lock);
            gI2CBusNum = SUPPORT_I2C_BUS_NUM1;
            spin_unlock(&kdsensor_drv_lock);
            PK_XLOG_INFO("kd_MultiSensorOpen: switch I2C BUS%d\n", gI2CBusNum);
        }
        else if (DUAL_CAMERA_SUB_3_SENSOR == g_invokeSocketIdx[i]) {
            spin_lock(&kdsensor_drv_lock);
#ifdef CUSTOM_DUMMY_AEON_SENSOR
            gI2CBusNum = SUPPORT_I2C_BUS_NUM3;
#else
            gI2CBusNum = SUPPORT_I2C_BUS_NUM2;
#endif
...
inline static int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
   ...
   else if (DUAL_CAMERA_MAIN_3_SENSOR == pSensorGetInfo->SensorId) {
	IDNum = 0;
    }
   else if (DUAL_CAMERA_SUB_3_SENSOR == pSensorGetInfo->SensorId) {
	IDNum = 1;
    }
    else
    {
    ...
    if ((DUAL_CAMERA_MAIN_SENSOR == pSensorGetInfo->SensorId) || (DUAL_CAMERA_MAIN_3_SENSOR == pSensorGetInfo->SensorId)){
    /* Resolution */
