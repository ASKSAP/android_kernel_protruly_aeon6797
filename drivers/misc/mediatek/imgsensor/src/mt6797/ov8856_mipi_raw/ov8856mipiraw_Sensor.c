...
static imgsensor_info_struct imgsensor_info = { 
	...
	.i2c_addr_table = {0x6c, 0xD8, 0x42, 0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
};
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			*sensor_id = ((read_cmos_sensor(0x7001) << 8) | read_cmos_sensor(0x7002));
			printk("ov8856 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			write_cmos_sensor(0x0100,0x00);
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	
				return ERROR_NONE;
				}
		//	LOG_INF("i2c fail id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
#define RG_Ratio_Typical   0x132
#define BG_Ratio_Typical   0x13a
...
//	LOG_INF("rg_ratio=%x ,bg_ratio=%x\n",(*otp_ptr).rg_ratio,(*otp_ptr).bg_ratio);
	...
	(* otp_ptr).VCM_end = 0;
	(* otp_ptr).VCM_dir = 0;
	printk("invalid VCM Calibration in OTP\n");
	}
	...
	printk("invalid Lenc Calibration in OTP\n");
	}
