...
#include "imx259mipiraw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "IMX259_camera_sensor"
#define LOG_1 LOG_INF("IMX259,MIPI 1LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX259_SENSOR_ID, //IMX259MIPI_SENSOR_ID,  /*sensor_id = 0x2680*/ //record sensor id defined in Kd_imgsensor.h
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gb, //SENSOR_OUTPUT_FORMAT_RAW_Gr,//SENSOR_OUTPUT_FORMAT_RAW_R,//sensor output first pixel color
...
#define IMX259_HDR_TYPE (0x00)
#define IMX259_BINNING_TYPE (0x10)
static kal_uint16 imx259_type = 0;/*0x00=HDR type, 0x10=binning type*/
/* Sensor output window information */
/*according toIMX259 datasheet p53 image cropping*/
...
#define IMX259MIPI_MaxGainIndex (154)
kal_uint16 IMX259MIPI_sensorGainMapping[IMX259MIPI_MaxGainIndex][2] ={
	{64 ,0	},
	{65 ,8	},
	{66 ,16 },
	{67 ,25 },
...
extern bool read_imx259_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size);
extern bool read_imx259_eeprom_SPC( kal_uint16 addr, BYTE* data, kal_uint32 size);

static kal_uint8 IMX259MIPI_SPC_Data[126];
static kal_uint8 SPC_data_done = false;
static void load_imx259_SPC_Data(void)
{
	kal_uint16 i;
	if ( SPC_data_done == false ) {
		if (!read_imx259_eeprom_SPC(0x0F73,IMX259MIPI_SPC_Data,126)) {
			LOG_INF("imx259 load spc fail\n");
			return;
		}
		SPC_data_done = true;
	}
	...
	for(i=0; i<63; i++)
	{
		write_cmos_sensor(0xD04C+i, IMX259MIPI_SPC_Data[i]);
		//LOG_INF("SPC_Data[%d] = %d\n", i, IMX259MIPI_SPC_Data[i]);
	}
	for(i=0; i<63; i++)
	{
		write_cmos_sensor(0xD08C+i, IMX259MIPI_SPC_Data[i+63]);
		//LOG_INF("SPC_Data[%d] = %d\n", i+63, IMX259MIPI_SPC_Data[i+63]);
	}
}
...
//	kal_uint32 tmp = 0;
	...
	if(write_cmos_sensor(0x0A02, 0x0F)==0){
		...
		while(retry--)
		{
			 ...
			 {
			 	 imx259_type = read_cmos_sensor(0x0A2E);
				 LOG_INF("imx259 type = 0x%x(0x00=HDR,0x10=binning)", imx259_type);
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
    ...
    for (i = 0; i < IMX259MIPI_MaxGainIndex; i++) {
        if(gain <= IMX259MIPI_sensorGainMapping[i][0]){
            break;
        }
    }
    if(gain != IMX259MIPI_sensorGainMapping[i][0])
            LOG_INF("Gain mapping don't correctly:%d %d \n", gain, IMX259MIPI_sensorGainMapping[i][0]);
    return IMX259MIPI_sensorGainMapping[i][1];
}
...
/*No Need to implement this function*/
	...
	/*	preview_setting  */
static void imx259_ImageQuality_Setting(void)
{
...
static void sensor_init_sync(void){
	extern int kdSensorPowerStatus(int SensorIdx);
	int powerOn=0;
	
	powerOn = kdSensorPowerStatus(0);
	
#ifdef IMX259_DEVICE_SYNC
	if(powerOn == 1 ){
		write_cmos_sensor(0x3004,0x00);
		write_cmos_sensor(0x5A5C,0x00);
		write_cmos_sensor(0x5A5D,0x00);
		write_cmos_sensor(0x4635,0x11);
		write_cmos_sensor(0x463b,0xff);
		write_cmos_sensor(0x5A5B,0X0F);
		write_cmos_sensor(0x5A90,0x00);
		write_cmos_sensor(0x5A92,0x08);
		write_cmos_sensor(0x5A93,0x00);
		write_cmos_sensor(0x5C0C,0x01);
		write_cmos_sensor(0x5C0D,0x07);
	}
#endif
}
...
static void sensor_init(void)
{
	...
	imx259_ImageQuality_Setting();
	write_cmos_sensor(0xa100,0x03);
	write_cmos_sensor(0xa101,0xff);
	write_cmos_sensor(0xa102,0x03);
	write_cmos_sensor(0xa103,0xff);
	write_cmos_sensor(0xa104,0x02);
	write_cmos_sensor(0xa105,0x96);
	write_cmos_sensor(0xa106,0x00);
	write_cmos_sensor(0xa107,0xf4);
	write_cmos_sensor(0xa108,0x00);
	write_cmos_sensor(0xa109,0xe7);
	write_cmos_sensor(0xa10a,0x00);
	write_cmos_sensor(0xa10b,0xef);
	write_cmos_sensor(0xa10c,0x01);
	write_cmos_sensor(0xa10d,0x6f);
	write_cmos_sensor(0xa10e,0x03);
	write_cmos_sensor(0xa10f,0xff);
	write_cmos_sensor(0xa110,0x03);
	write_cmos_sensor(0xa111,0xff);
	write_cmos_sensor(0xa112,0x03);
	write_cmos_sensor(0xa113,0xff);
	write_cmos_sensor(0xa114,0x00);
	write_cmos_sensor(0xa115,0xe3);
	write_cmos_sensor(0xa116,0x01);
	write_cmos_sensor(0xa117,0x2A);
	write_cmos_sensor(0xa118,0x00);
	write_cmos_sensor(0xa119,0xcE);
	write_cmos_sensor(0xa11a,0x00);
	write_cmos_sensor(0xa11b,0xc9);
	write_cmos_sensor(0xa11c,0x00);
	write_cmos_sensor(0xa11d,0xd3);
	write_cmos_sensor(0xa11e,0x01);
	write_cmos_sensor(0xa11f,0x09);
	write_cmos_sensor(0xa120,0x01);
	write_cmos_sensor(0xa121,0x63);
	write_cmos_sensor(0xa122,0x03);
	write_cmos_sensor(0xa123,0xff);
	write_cmos_sensor(0xa124,0x03);
	write_cmos_sensor(0xa125,0xff);
	write_cmos_sensor(0xa126,0x01);
	write_cmos_sensor(0xa127,0x4E);
	write_cmos_sensor(0xa128,0x00);
	write_cmos_sensor(0xa129,0xE2);
	write_cmos_sensor(0xa12a,0x00);
	write_cmos_sensor(0xa12b,0xEE);
	write_cmos_sensor(0xa12c,0x00);
	write_cmos_sensor(0xa12d,0xF6);
	write_cmos_sensor(0xa12e,0x00);
	write_cmos_sensor(0xa12f,0xEC);
	write_cmos_sensor(0xa130,0x00);
	write_cmos_sensor(0xa131,0xE9);
	write_cmos_sensor(0xa132,0x00);
	write_cmos_sensor(0xa133,0xe3);
	write_cmos_sensor(0xa134,0x03);
	write_cmos_sensor(0xa135,0xff);
	write_cmos_sensor(0xa136,0x03);
	write_cmos_sensor(0xa137,0xff);
	write_cmos_sensor(0xa138,0x01);
	write_cmos_sensor(0xa139,0x37);
	write_cmos_sensor(0xa13a,0x00);
	write_cmos_sensor(0xa13b,0xDD);
	write_cmos_sensor(0xa13c,0x00);
	write_cmos_sensor(0xa13d,0xF0);
	write_cmos_sensor(0xa13e,0x01);
	write_cmos_sensor(0xa13f,0x00);
	write_cmos_sensor(0xa140,0x00);
	write_cmos_sensor(0xa141,0xF1);
	write_cmos_sensor(0xa142,0x00);
	write_cmos_sensor(0xa143,0xDD);
	write_cmos_sensor(0xa144,0x01);
	write_cmos_sensor(0xa145,0x41);
	write_cmos_sensor(0xa146,0x02);
	write_cmos_sensor(0xa147,0xff);
	write_cmos_sensor(0xa148,0x03);
	write_cmos_sensor(0xa149,0xff);
	write_cmos_sensor(0xa14a,0x01);
	write_cmos_sensor(0xa14b,0x69);
	write_cmos_sensor(0xa14c,0x00);
	write_cmos_sensor(0xa14d,0xDF);
	write_cmos_sensor(0xa14e,0x00);
	write_cmos_sensor(0xa14f,0xE4);
	write_cmos_sensor(0xa150,0x00);
	write_cmos_sensor(0xa151,0xEC);
	write_cmos_sensor(0xa152,0x00);
	write_cmos_sensor(0xa153,0xE4);
	write_cmos_sensor(0xa154,0x00);
	write_cmos_sensor(0xa155,0xE0);
	write_cmos_sensor(0xa156,0x01);
	write_cmos_sensor(0xa157,0x3D);
	write_cmos_sensor(0xa158,0x02);
	write_cmos_sensor(0xa159,0xff);
	write_cmos_sensor(0xa15a,0x03);
	write_cmos_sensor(0xa15b,0xff);
	write_cmos_sensor(0xa15c,0x00);
	write_cmos_sensor(0xa15d,0xe3);
	write_cmos_sensor(0xa15e,0x01);
	write_cmos_sensor(0xa15f,0x74);
	write_cmos_sensor(0xa160,0x00);
	write_cmos_sensor(0xa161,0xDB);
	write_cmos_sensor(0xa162,0x00);
	write_cmos_sensor(0xa163,0xD1);
	write_cmos_sensor(0xa164,0x00);
	write_cmos_sensor(0xa165,0xE0);
	write_cmos_sensor(0xa166,0x01);
	write_cmos_sensor(0xa167,0x3A);
	write_cmos_sensor(0xa168,0x00);
	write_cmos_sensor(0xa169,0xe3);
	write_cmos_sensor(0xa16a,0x02);
	write_cmos_sensor(0xa16b,0xff);
	write_cmos_sensor(0xa16c,0x03);
	write_cmos_sensor(0xa16d,0xff);
	write_cmos_sensor(0xa16e,0x03);
	write_cmos_sensor(0xa16f,0xff);
	write_cmos_sensor(0xa170,0x02);
	write_cmos_sensor(0xa171,0xe0);
	write_cmos_sensor(0xa172,0x01);
	write_cmos_sensor(0xa173,0x4E);
	write_cmos_sensor(0xa174,0x01);
	write_cmos_sensor(0xa175,0x1D);
	write_cmos_sensor(0xa176,0x01);
	write_cmos_sensor(0xa177,0x4A);
	write_cmos_sensor(0xa178,0x01);
	write_cmos_sensor(0xa179,0xff);
	write_cmos_sensor(0xa17a,0x03);
	write_cmos_sensor(0xa17b,0xff);
	write_cmos_sensor(0xa17c,0x02);
	write_cmos_sensor(0xa17d,0xff);
	write_cmos_sensor(0xa17e,0x03);
	write_cmos_sensor(0xa17f,0xff);
	write_cmos_sensor(0xa180,0x03);
	write_cmos_sensor(0xa181,0xff);
	write_cmos_sensor(0xa182,0x03);
	write_cmos_sensor(0xa183,0xb0);
	write_cmos_sensor(0xa184,0x01);
	write_cmos_sensor(0xa185,0x65);
	write_cmos_sensor(0xa186,0x01);
	write_cmos_sensor(0xa187,0x63);
	write_cmos_sensor(0xa188,0x01);
	write_cmos_sensor(0xa189,0x63);
	write_cmos_sensor(0xa18a,0x01);
	write_cmos_sensor(0xa18b,0xe0);
	write_cmos_sensor(0xa18c,0x03);
	write_cmos_sensor(0xa18d,0xff);
	write_cmos_sensor(0xa18e,0x03);
	write_cmos_sensor(0xa18f,0xff);
	write_cmos_sensor(0xa190,0x03);
	write_cmos_sensor(0xa191,0xff);
	write_cmos_sensor(0xa192,0x01);
	write_cmos_sensor(0xa193,0x32);
	write_cmos_sensor(0xa194,0x01);
	write_cmos_sensor(0xa195,0x85);
	write_cmos_sensor(0xa196,0x01);
	write_cmos_sensor(0xa197,0x24);
	write_cmos_sensor(0xa198,0x01);
	write_cmos_sensor(0xa199,0x11);
	write_cmos_sensor(0xa19a,0x01);
	write_cmos_sensor(0xa19b,0x2D);
	write_cmos_sensor(0xa19c,0x01);
	write_cmos_sensor(0xa19d,0x89);
	write_cmos_sensor(0xa19e,0x02);
	write_cmos_sensor(0xa19f,0x14);
	write_cmos_sensor(0xa1a0,0x03);
	write_cmos_sensor(0xa1a1,0xff);
	write_cmos_sensor(0xa1a2,0x03);
	write_cmos_sensor(0xa1a3,0xff);
	write_cmos_sensor(0xa1a4,0x01);
	write_cmos_sensor(0xa1a5,0xE2);
	write_cmos_sensor(0xa1a6,0x01);
	write_cmos_sensor(0xa1a7,0x2B);
	write_cmos_sensor(0xa1a8,0x01);
	write_cmos_sensor(0xa1a9,0x06);
	write_cmos_sensor(0xa1aa,0x01);
	write_cmos_sensor(0xa1ab,0x02);
	write_cmos_sensor(0xa1ac,0x01);
	write_cmos_sensor(0xa1ad,0x09);
	write_cmos_sensor(0xa1ae,0x01);
	write_cmos_sensor(0xa1af,0x41);
	write_cmos_sensor(0xa1b0,0x01);
	write_cmos_sensor(0xa1b1,0x8a);
	write_cmos_sensor(0xa1b2,0x03);
	write_cmos_sensor(0xa1b3,0xff);
	write_cmos_sensor(0xa1b4,0x03);
	write_cmos_sensor(0xa1b5,0xff);
	write_cmos_sensor(0xa1b6,0x01);
	write_cmos_sensor(0xa1b7,0xDA);
	write_cmos_sensor(0xa1b8,0x01);
	write_cmos_sensor(0xa1b9,0x26);
	write_cmos_sensor(0xa1ba,0x01);
	write_cmos_sensor(0xa1bb,0x03);
	write_cmos_sensor(0xa1bc,0x01);
	write_cmos_sensor(0xa1bd,0x00);
	write_cmos_sensor(0xa1be,0x01);
	write_cmos_sensor(0xa1bf,0x06);
	write_cmos_sensor(0xa1c0,0x01);
	write_cmos_sensor(0xa1c1,0x31);
	write_cmos_sensor(0xa1c2,0x01);
	write_cmos_sensor(0xa1c3,0xFC);
	write_cmos_sensor(0xa1c4,0x03);
	write_cmos_sensor(0xa1c5,0xff);
	write_cmos_sensor(0xa1c6,0x03);
	write_cmos_sensor(0xa1c7,0xff);
	write_cmos_sensor(0xa1c8,0x02);
	write_cmos_sensor(0xa1c9,0x18);
	write_cmos_sensor(0xa1ca,0x01);
	write_cmos_sensor(0xa1cb,0x30);
	write_cmos_sensor(0xa1cc,0x01);
	write_cmos_sensor(0xa1cd,0x09);
	write_cmos_sensor(0xa1ce,0x01);
	write_cmos_sensor(0xa1cf,0x03);
	write_cmos_sensor(0xa1d0,0x01);
	write_cmos_sensor(0xa1d1,0x0C);
	write_cmos_sensor(0xa1d2,0x01);
	write_cmos_sensor(0xa1d3,0x3E);
	write_cmos_sensor(0xa1d4,0x01);
	write_cmos_sensor(0xa1d5,0xE5);
	write_cmos_sensor(0xa1d6,0x03);
	write_cmos_sensor(0xa1d7,0xff);
	write_cmos_sensor(0xa1d8,0x03);
	write_cmos_sensor(0xa1d9,0xff);
	write_cmos_sensor(0xa1da,0x01);
	write_cmos_sensor(0xa1db,0x32);
	write_cmos_sensor(0xa1dc,0x02);
	write_cmos_sensor(0xa1dd,0x11);
	write_cmos_sensor(0xa1de,0x01);
	write_cmos_sensor(0xa1df,0x24);
	write_cmos_sensor(0xa1e0,0x01);
	write_cmos_sensor(0xa1e1,0x12);
	write_cmos_sensor(0xa1e2,0x01);
	write_cmos_sensor(0xa1e3,0x2D);
	write_cmos_sensor(0xa1e4,0x01);
	write_cmos_sensor(0xa1e5,0xEC);
	write_cmos_sensor(0xa1e6,0x01);
	write_cmos_sensor(0xa1e7,0x32);
	write_cmos_sensor(0xa1e8,0x03);
	write_cmos_sensor(0xa1e9,0xff);
	write_cmos_sensor(0xa1ea,0x03);
	write_cmos_sensor(0xa1eb,0xff);
	write_cmos_sensor(0xa1ec,0x03);
	write_cmos_sensor(0xa1ed,0xff);
	write_cmos_sensor(0xa1ee,0x03);
	write_cmos_sensor(0xa1ef,0xff);
	write_cmos_sensor(0xa1f0,0x01);
	write_cmos_sensor(0xa1f1,0xD4);
	write_cmos_sensor(0xa1f2,0x01);
	write_cmos_sensor(0xa1f3,0x96);
	write_cmos_sensor(0xa1f4,0x01);
	write_cmos_sensor(0xa1f5,0xD4);
	write_cmos_sensor(0xa1f6,0x02);
	write_cmos_sensor(0xa1f7,0xff);
	write_cmos_sensor(0xa1f8,0x03);
	write_cmos_sensor(0xa1f9,0xff);
	write_cmos_sensor(0xa1fa,0x03);
	write_cmos_sensor(0xa1fb,0xff);
	write_cmos_sensor(0xa1fc,0x03);
	write_cmos_sensor(0xa1fd,0xff);
	write_cmos_sensor(0xa1fe,0x03);
	write_cmos_sensor(0xa1ff,0xff);
	write_cmos_sensor(0xa200,0x03);
	write_cmos_sensor(0xa201,0xb0);
	write_cmos_sensor(0xa202,0x01);
	write_cmos_sensor(0xa203,0x73);
	write_cmos_sensor(0xa204,0x01);
	write_cmos_sensor(0xa205,0x7A);
	write_cmos_sensor(0xa206,0x01);
	write_cmos_sensor(0xa207,0x8E);
	write_cmos_sensor(0xa208,0x01);
	write_cmos_sensor(0xa209,0xe0);
	write_cmos_sensor(0xa20a,0x03);
	write_cmos_sensor(0xa20b,0xff);
	write_cmos_sensor(0xa20c,0x03);
	write_cmos_sensor(0xa20d,0xff);
	write_cmos_sensor(0xa20e,0x03);
	write_cmos_sensor(0xa20f,0xff);
	write_cmos_sensor(0xa210,0x01);
	write_cmos_sensor(0xa211,0x32);
	write_cmos_sensor(0xa212,0x01);
	write_cmos_sensor(0xa213,0x8F);
	write_cmos_sensor(0xa214,0x01);
	write_cmos_sensor(0xa215,0x35);
	write_cmos_sensor(0xa216,0x01);
	write_cmos_sensor(0xa217,0x26);
	write_cmos_sensor(0xa218,0x01);
	write_cmos_sensor(0xa219,0x3F);
	write_cmos_sensor(0xa21a,0x01);
	write_cmos_sensor(0xa21b,0x8C);
	write_cmos_sensor(0xa21c,0x02);
	write_cmos_sensor(0xa21d,0x14);
	write_cmos_sensor(0xa21e,0x03);
	write_cmos_sensor(0xa21f,0xff);
	write_cmos_sensor(0xa220,0x03);
	write_cmos_sensor(0xa221,0xff);
	write_cmos_sensor(0xa222,0x01);
	write_cmos_sensor(0xa223,0xBE);
	write_cmos_sensor(0xa224,0x01);
	write_cmos_sensor(0xa225,0x1E);
	write_cmos_sensor(0xa226,0x01);
	write_cmos_sensor(0xa227,0x07);
	write_cmos_sensor(0xa228,0x01);
	write_cmos_sensor(0xa229,0x07);
	write_cmos_sensor(0xa22a,0x01);
	write_cmos_sensor(0xa22b,0x0A);
	write_cmos_sensor(0xa22c,0x01);
	write_cmos_sensor(0xa22d,0x2F);
	write_cmos_sensor(0xa22e,0x01);
	write_cmos_sensor(0xa22f,0x83);
	write_cmos_sensor(0xa230,0x03);
	write_cmos_sensor(0xa231,0xff);
	write_cmos_sensor(0xa232,0x03);
	write_cmos_sensor(0xa233,0xff);
	write_cmos_sensor(0xa234,0x01);
	write_cmos_sensor(0xa235,0xA7);
	write_cmos_sensor(0xa236,0x01);
	write_cmos_sensor(0xa237,0x14);
	write_cmos_sensor(0xa238,0x01);
	write_cmos_sensor(0xa239,0x00);
	write_cmos_sensor(0xa23a,0x01);
	write_cmos_sensor(0xa23b,0x00);
	write_cmos_sensor(0xa23c,0x01);
	write_cmos_sensor(0xa23d,0x03);
	write_cmos_sensor(0xa23e,0x01);
	write_cmos_sensor(0xa23f,0x1B);
	write_cmos_sensor(0xa240,0x01);
	write_cmos_sensor(0xa241,0xBF);
	write_cmos_sensor(0xa242,0x03);
	write_cmos_sensor(0xa243,0xff);
	write_cmos_sensor(0xa244,0x03);
	write_cmos_sensor(0xa245,0xff);
	write_cmos_sensor(0xa246,0x01);
	write_cmos_sensor(0xa247,0xE9);
	write_cmos_sensor(0xa248,0x01);
	write_cmos_sensor(0xa249,0x21);
	write_cmos_sensor(0xa24a,0x01);
	write_cmos_sensor(0xa24b,0x08);
	write_cmos_sensor(0xa24c,0x01);
	write_cmos_sensor(0xa24d,0x06);
	write_cmos_sensor(0xa24e,0x01);
	write_cmos_sensor(0xa24f,0x0B);
	write_cmos_sensor(0xa250,0x01);
	write_cmos_sensor(0xa251,0x2A);
	write_cmos_sensor(0xa252,0x01);
	write_cmos_sensor(0xa253,0xB8);
	write_cmos_sensor(0xa254,0x03);
	write_cmos_sensor(0xa255,0xff);
	write_cmos_sensor(0xa256,0x03);
	write_cmos_sensor(0xa257,0xff);
	write_cmos_sensor(0xa258,0x01);
	write_cmos_sensor(0xa259,0x32);
	write_cmos_sensor(0xa25a,0x02);
	write_cmos_sensor(0xa25b,0x0F);
	write_cmos_sensor(0xa25c,0x01);
	write_cmos_sensor(0xa25d,0x34);
	write_cmos_sensor(0xa25e,0x01);
	write_cmos_sensor(0xa25f,0x26);
	write_cmos_sensor(0xa260,0x01);
	write_cmos_sensor(0xa261,0x3D);
	write_cmos_sensor(0xa262,0x01);
	write_cmos_sensor(0xa263,0xE5);
	write_cmos_sensor(0xa264,0x01);
	write_cmos_sensor(0xa265,0x32);
	write_cmos_sensor(0xa266,0x03);
	write_cmos_sensor(0xa267,0xff);
	write_cmos_sensor(0xa268,0x03);
	write_cmos_sensor(0xa269,0xff);
	write_cmos_sensor(0xa26a,0x03);
	write_cmos_sensor(0xa26b,0xff);
	write_cmos_sensor(0xa26c,0x03);
	write_cmos_sensor(0xa26d,0xff);
	write_cmos_sensor(0xa26e,0x02);
	write_cmos_sensor(0xa26f,0x0C);
	write_cmos_sensor(0xa270,0x01);
	write_cmos_sensor(0xa271,0xCC);
	write_cmos_sensor(0xa272,0x02);
	write_cmos_sensor(0xa273,0x07);
	write_cmos_sensor(0xa274,0x02);
	write_cmos_sensor(0xa275,0xff);
	write_cmos_sensor(0xa276,0x03);
	write_cmos_sensor(0xa277,0xff);
	write_cmos_sensor(0xa278,0x03);
	write_cmos_sensor(0xa279,0xff);
	write_cmos_sensor(0xa27a,0x03);
	write_cmos_sensor(0xa27b,0xff);
	write_cmos_sensor(0xa27c,0x03);
	write_cmos_sensor(0xa27d,0xff);
	write_cmos_sensor(0xa27e,0x03);
	write_cmos_sensor(0xa27f,0x80);
	write_cmos_sensor(0xa280,0x01);
	write_cmos_sensor(0xa281,0x31);
	write_cmos_sensor(0xa282,0x01);
	write_cmos_sensor(0xa283,0x30);
	write_cmos_sensor(0xa284,0x01);
	write_cmos_sensor(0xa285,0x2C);
	write_cmos_sensor(0xa286,0x01);
	write_cmos_sensor(0xa287,0x70);
	write_cmos_sensor(0xa288,0x03);
	write_cmos_sensor(0xa289,0xff);
	write_cmos_sensor(0xa28a,0x03);
	write_cmos_sensor(0xa28b,0xff);
	write_cmos_sensor(0xa28c,0x03);
	write_cmos_sensor(0xa28d,0xff);
	write_cmos_sensor(0xa28e,0x01);
	write_cmos_sensor(0xa28f,0x32);
	write_cmos_sensor(0xa290,0x01);
	write_cmos_sensor(0xa291,0x51);
	write_cmos_sensor(0xa292,0x01);
	write_cmos_sensor(0xa293,0x07);
	write_cmos_sensor(0xa294,0x00);
	write_cmos_sensor(0xa295,0xFC);
	write_cmos_sensor(0xa296,0x01);
	write_cmos_sensor(0xa297,0x0D);
	write_cmos_sensor(0xa298,0x01);
	write_cmos_sensor(0xa299,0x4F);
	write_cmos_sensor(0xa29a,0x01);
	write_cmos_sensor(0xa29b,0xe2);
	write_cmos_sensor(0xa29c,0x03);
	write_cmos_sensor(0xa29d,0xff);
	write_cmos_sensor(0xa29e,0x03);
	write_cmos_sensor(0xa29f,0xff);
	write_cmos_sensor(0xa2a0,0x01);
	write_cmos_sensor(0xa2a1,0x89);
	write_cmos_sensor(0xa2a2,0x01);
	write_cmos_sensor(0xa2a3,0x00);
	write_cmos_sensor(0xa2a4,0x00);
	write_cmos_sensor(0xa2a5,0xF7);
	write_cmos_sensor(0xa2a6,0x00);
	write_cmos_sensor(0xa2a7,0xFC);
	write_cmos_sensor(0xa2a8,0x00);
	write_cmos_sensor(0xa2a9,0xF8);
	write_cmos_sensor(0xa2aa,0x01);
	write_cmos_sensor(0xa2ab,0x0C);
	write_cmos_sensor(0xa2ac,0x01);
	write_cmos_sensor(0xa2ad,0x3c);
	write_cmos_sensor(0xa2ae,0x03);
	write_cmos_sensor(0xa2af,0xff);
	write_cmos_sensor(0xa2b0,0x03);
	write_cmos_sensor(0xa2b1,0xff);
	write_cmos_sensor(0xa2b2,0x01);
	write_cmos_sensor(0xa2b3,0x75);
	write_cmos_sensor(0xa2b4,0x00);
	write_cmos_sensor(0xa2b5,0xFE);
	write_cmos_sensor(0xa2b6,0x00);
	write_cmos_sensor(0xa2b7,0xF8);
	write_cmos_sensor(0xa2b8,0x01);
	write_cmos_sensor(0xa2b9,0x00);
	write_cmos_sensor(0xa2ba,0x00);
	write_cmos_sensor(0xa2bb,0xF9);
	write_cmos_sensor(0xa2bc,0x00);
	write_cmos_sensor(0xa2bd,0xFF);
	write_cmos_sensor(0xa2be,0x01);
	write_cmos_sensor(0xa2bf,0x83);
	write_cmos_sensor(0xa2c0,0x03);
	write_cmos_sensor(0xa2c1,0xff);
	write_cmos_sensor(0xa2c2,0x03);
	write_cmos_sensor(0xa2c3,0xff);
	write_cmos_sensor(0xa2c4,0x01);
	write_cmos_sensor(0xa2c5,0xA9);
	write_cmos_sensor(0xa2c6,0x01);
	write_cmos_sensor(0xa2c7,0x01);
	write_cmos_sensor(0xa2c8,0x00);
	write_cmos_sensor(0xa2c9,0xF5);
	write_cmos_sensor(0xa2ca,0x00);
	write_cmos_sensor(0xa2cb,0xF6);
	write_cmos_sensor(0xa2cc,0x00);
	write_cmos_sensor(0xa2cd,0xF6);
	write_cmos_sensor(0xa2ce,0x01);
	write_cmos_sensor(0xa2cf,0x06);
	write_cmos_sensor(0xa2d0,0x01);
	write_cmos_sensor(0xa2d1,0x7A);
	write_cmos_sensor(0xa2d2,0x03);
	write_cmos_sensor(0xa2d3,0xff);
	write_cmos_sensor(0xa2d4,0x03);
	write_cmos_sensor(0xa2d5,0xff);
	write_cmos_sensor(0xa2d6,0x01);
	write_cmos_sensor(0xa2d7,0x32);
	write_cmos_sensor(0xa2d8,0x01);
	write_cmos_sensor(0xa2d9,0xB7);
	write_cmos_sensor(0xa2da,0x01);
	write_cmos_sensor(0xa2db,0x02);
	write_cmos_sensor(0xa2dc,0x00);
	write_cmos_sensor(0xa2dd,0xF7);
	write_cmos_sensor(0xa2de,0x01);
	write_cmos_sensor(0xa2df,0x0A);
	write_cmos_sensor(0xa2e0,0x01);
	write_cmos_sensor(0xa2e1,0x9D);
	write_cmos_sensor(0xa2e2,0x01);
	write_cmos_sensor(0xa2e3,0x0e);
	write_cmos_sensor(0xa2e4,0x03);
	write_cmos_sensor(0xa2e5,0xff);
	write_cmos_sensor(0xa2e6,0x03);
	write_cmos_sensor(0xa2e7,0xff);
	write_cmos_sensor(0xa2e8,0x03);
	write_cmos_sensor(0xa2e9,0xff);
	write_cmos_sensor(0xa2ea,0x03);
	write_cmos_sensor(0xa2eb,0xa0);
	write_cmos_sensor(0xa2ec,0x01);
	write_cmos_sensor(0xa2ed,0x9B);
	write_cmos_sensor(0xa2ee,0x01);
	write_cmos_sensor(0xa2ef,0x62);
	write_cmos_sensor(0xa2f0,0x01);
	write_cmos_sensor(0xa2f1,0x9A);
	write_cmos_sensor(0xa2f2,0x01);
	write_cmos_sensor(0xa2f3,0xff);
	write_cmos_sensor(0xa2f4,0x03);
	write_cmos_sensor(0xa2f5,0xff);
	write_cmos_sensor(0xa2f6,0x03);
	write_cmos_sensor(0xa2f7,0xff);
	//write_cmos_sensor(0x7b8c,0x01);
	//write_cmos_sensor(0x7b8d,0x01);
	//write_cmos_sensor(0x7b8e,0x01);
	//write_cmos_sensor(0x7b8f,0x01);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	s_DEVINFO_ccm->device_ic = "IMX259";
  ...
  if(!((feature_id == 3004) || (feature_id == 3006)))
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			...
			read_imx259_eeprom((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
...
UINT32 IMX259_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
/*UINT32 IMX259_MIPI_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)*/
