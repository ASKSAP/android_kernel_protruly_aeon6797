...
#define PFX "OV13858_camera_sensor"
...
int ov13858_chip_ver = OV13858_R2A;
//for test 
int m_bpc_select=0;
//sensor otp
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV13858_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static imgsensor_struct imgsensor = {
    .mirror = IMAGE_V_MIRROR,                //mirrorflip information IMAGE_V_MIRROR
...
//add from OV13858_v1_mirror_on_flip_off_mtk2.0.1.ini
static SET_PD_BLOCK_INFO_T imgsensor_pd_info =
{
    ...
    .i4PosL = {{86,70},{102,70},{78,74},{94,74},{86,86},{102,86},{78,90},{94,90}},
    .i4PosR = {{86,66},{102,66},{78,78},{94,78},{86,82},{102,82},{78,94},{94,94}},
};
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	return iReg;//ov13858. sensorGlobalGain
}
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x37e4, 0x38);//0x34
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
    ...
    if (enable) {
        ...
        write_cmos_sensor(0x5000, 0xd9);//disable lenc and otp_dpc
...
/* HTC_START */
//extern bool OV13858_read_eeprom_module_info(BYTE* data);

//#define OTP_DATA_SIZE 32
//static char ov13858_otp_data[OTP_DATA_SIZE];
//static int read_ov13858_otp_flag = 0;
/*
static int read_ov13858_otp(void){
	int ret = 0;

	LOG_INF("==========read_ov13858_otp_flag =%d=======\n",read_ov13858_otp_flag);
	if(read_ov13858_otp_flag !=0){
		LOG_INF("==========ov13858 otp readed=======\n");
		return 0;
	}

	memset(ov13858_otp_data, 0x0, OTP_DATA_SIZE);
	ret = OV13858_read_eeprom_module_info(ov13858_otp_data);
#if 1
	LOG_INF("OTP Module Vendor = 0x%x\n",                    ov13858_otp_data[0x00]);
	LOG_INF("OTP Lens Vendor = 0x%x\n",                      ov13858_otp_data[0x01]);
	LOG_INF("OTP Lens Rev = 0x%x\n",                         ov13858_otp_data[0x02]);
	LOG_INF("OTP Sensor Vendor = 0x%x\n",                    ov13858_otp_data[0x03]);
	LOG_INF("OTP Sensor Rev = 0x%x\n",                       ov13858_otp_data[0x04]);
	LOG_INF("OTP Drive IC Info = 0x%x\n",                    ov13858_otp_data[0x05]);
	LOG_INF("OTP Drive IC Rev = 0x%x\n",                     ov13858_otp_data[0x06]);
	LOG_INF("OTP Actuator Info = 0x%x\n",                    ov13858_otp_data[0x07]);
	LOG_INF("OTP Actuator Rev = 0x%x\n",                     ov13858_otp_data[0x08]);
	LOG_INF("OTP EEPROM vendor = 0x%x\n",                    ov13858_otp_data[0x09]);
	LOG_INF("OTP EEPROM Rev = 0x%x\n",                       ov13858_otp_data[0x0A]);
	LOG_INF("OTP PCB vendor = 0x%x\n",                       ov13858_otp_data[0x0B]);
	LOG_INF("OTP PCB Rev = 0x%x\n",                          ov13858_otp_data[0x0C]);
	LOG_INF("OTP Module Serial Number = 0x%x 0x%x 0x%x 0x%x\n",
		ov13858_otp_data[0x0D], ov13858_otp_data[0x0E], ov13858_otp_data[0x0F], ov13858_otp_data[0x10]);

	LOG_INF("OTP AF Infinity Position (High byte) = 0x%x\n", ov13858_otp_data[0x13]);
	LOG_INF("OTP AF Infinity Position (Low byte) = 0x%x\n",  ov13858_otp_data[0x14]);
	LOG_INF("OTP AF Macro Position (High byte) = 0x%x\n",    ov13858_otp_data[0x15]);
	LOG_INF("OTP AF Macro Position (Low byte) = 0x%x\n",     ov13858_otp_data[0x16]);
#endif

	if(ret == true) {
		spin_lock(&imgsensor_drv_lock);
		read_ov13858_otp_flag = 1;
		spin_unlock(&imgsensor_drv_lock);
	}

	LOG_INF("==========exit ov13858 read_otp=======\n");
	return ret;
}
...
static ssize_t sensor_otp_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	LOG_INF("OTP AF Macro Position (High byte) = 0x%x\n",    ov13858_otp_data[0x15]);
	LOG_INF("OTP AF Macro Position (Low byte) = 0x%x\n",    ov13858_otp_data[0x16]);
	inf_val = (ov13858_otp_data[0x13]<<8 | ov13858_otp_data[0x14]);
	mac_val = (ov13858_otp_data[0x15]<<8 | ov13858_otp_data[0x16]);
...
static const char *ov13858Vendor = "OmniVision";
static const char *ov13858NAME = "ov13858_front";
static const char *ov13858Size = "13.0M";
		...
		struct device_attribute *attr, char *buf)
{
	...
	sprintf(buf, "%s %s %s\n", ov13858Vendor, ov13858NAME, ov13858Size);
	ret = strlen(buf) + 1;
...
static struct kobject *android_ov13858;
static int first = true;

static int ov13858_sysfs_init(void)
{
	int ret ;
	if(first){
		LOG_INF("kobject creat and add\n");
		android_ov13858 = kobject_create_and_add("android_camera2", NULL);
		if (android_ov13858 == NULL) {
		...
		ret = sysfs_create_file(android_ov13858, &dev_attr_sensor.attr);
		if (ret) {
			LOG_INF("sysfs_create_file " \
			"failed\n");
			kobject_del(android_ov13858);
		}else
			first = false;

		ret = sysfs_create_file(android_ov13858, &dev_attr_otp.attr);
		if (ret) {
			LOG_INF("sysfs_create_file " \
			"failed\n");
			kobject_del(android_ov13858);
        ...
        do {
            *sensor_id = return_sensor_id();
            ...
            if (*sensor_id == imgsensor_info.sensor_id) {
              ...
              //  ov13858_sysfs_init();
         /*       read_ov13858_otp();
                SensorType=read_cmos_sensor(0x36f0);
                if(SensorType==0x06)
                {
                  s_DEVINFO_ccm->device_ic = "OV13858&MP ";
                }
                else
                {
                 s_DEVINFO_ccm->device_ic = "OV13858&ES ";
                }
                #ifdef SLT_DEVINFO_CMM_OV13858_OFILM 
...
static kal_uint32 open(void)
{
    ...
    if (imgsensor_info.sensor_id != sensor_id)
		...
		{
				LOG_INF("----R1A---- \n");
				ov13858_chip_ver = OV13858_R1A;
		}else if((read_cmos_sensor(0x302a))==0xb2)
		{
				LOG_INF("----R2A---- \n");
				ov13858_chip_ver = OV13858_R2A;
		}
    /* initail sequence write in  */
    sensor_init();
	 //for OTP
	 LOG_INF("OV13858 OTP Loading Start\n");
	  LOG_INF("OTP Loading i2c write id: 0x%x\n", imgsensor.i2c_write_id);
	//otp_TestValue = read_cmos_sensor(0x3501);
  //	LOG_INF("otp_TestValue03: %x \n", otp_TestValue);
  //	write_cmos_sensor(0x3501, 0x82);
//	 otp_TestValue = read_cmos_sensor(0x3501);
  //	LOG_INF("otp_TestValue04: %x \n", otp_TestValue);
	 otp_cali(imgsensor.i2c_write_id);
	 LOG_INF("OV13858 OTP Loading End\n");
	 write_cmos_sensor(0x0100, 0x00);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
            ...
            //PDAF capacity enable or not, ov13858 only full size support PDAF
            switch (*feature_data) {
                case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
...
UINT32 OV13858_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    OV13858_MIPI_RAW_SensorInit    */
