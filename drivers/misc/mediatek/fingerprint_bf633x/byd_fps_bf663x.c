...
extern unsigned char byd_fps_susflag;
...
#if (defined(BYD_FPS_REPORT_KEY))||(defined(BYD_FPS_FOCUS))||(defined(BYD_FPS_REPORT_KEY_ONLY))
...
extern unsigned short otp_fpd_th;
const unsigned int BYD_FPS_SPI_CLOCK_SPEED = BYD_FPS_SPI_SPEED;
const unsigned int BYD_FPS_DEFAULT_IRQ_TIMEOUT =	(HZ/2);	//200; //((10/1000) * HZ)

unsigned char BYD_FPS_IMAGE_WIDTH = BYD_FPS_SENSOR_WIDTH11;
unsigned char BYD_FPS_IMAGE_HEIGHT = BYD_FPS_SENSOR_HEIGHT11;
unsigned char BYD_FPS_DATA_WIDTH = BYD_FPS_SENSOR_WIDTH11*BYD_FPS_DATA_BYTE;
unsigned char BYD_FPS_DATA_HEIGHT = BYD_FPS_SENSOR_HEIGHT11;
unsigned int BYD_FPS_IMAGE_SIZE = (BYD_FPS_SENSOR_WIDTH11)*(BYD_FPS_SENSOR_HEIGHT11)*BYD_FPS_DATA_BYTE;
unsigned int BYD_FPS_IMAGE_BUFFER_SIZE = ((BYD_FPS_SENSOR_WIDTH11)*2+(BYD_FPS_SPI_DUMMY_NUM))*(BYD_FPS_SENSOR_HEIGHT11)+1;

const unsigned char BYD_FPS_IMAGE_WIDTH11 = BYD_FPS_SENSOR_WIDTH11;
const unsigned char BYD_FPS_IMAGE_HEIGHT11 = BYD_FPS_SENSOR_HEIGHT11;
const unsigned int BYD_FPS_IMAGE_SIZE11 = (BYD_FPS_SENSOR_WIDTH11)*(BYD_FPS_SENSOR_HEIGHT11)*BYD_FPS_DATA_BYTE;
const unsigned int BYD_FPS_IMAGE_BUFFER_SIZE11 = ((BYD_FPS_SENSOR_WIDTH11)*2+(BYD_FPS_SPI_DUMMY_NUM))*(BYD_FPS_SENSOR_HEIGHT11)+1;

const unsigned char BYD_FPS_IMAGE_WIDTH12 = BYD_FPS_SENSOR_WIDTH12;
const unsigned char BYD_FPS_IMAGE_HEIGHT12 = BYD_FPS_SENSOR_HEIGHT12;
const unsigned int BYD_FPS_IMAGE_SIZE12 = (BYD_FPS_SENSOR_WIDTH12)*(BYD_FPS_SENSOR_HEIGHT12)*BYD_FPS_DATA_BYTE;
const unsigned int BYD_FPS_IMAGE_BUFFER_SIZE12 = ((BYD_FPS_SENSOR_WIDTH12)*2+(BYD_FPS_SPI_DUMMY_NUM))*(BYD_FPS_SENSOR_HEIGHT12)+1;
...
#ifdef CONFIG_FPS12
const unsigned int BYD_FPS_CONFIG_VERSION12 = CONFIG_FPS12;	//设置芯片版本号,见byd_fps_bf66xx.h
#endif

#ifdef CONFIG_FPS11
const unsigned int BYD_FPS_CONFIG_VERSION11 = CONFIG_FPS11;	//设置芯片版本号,见byd_fps_bf66xx.h
#endif

#ifdef CONFIG_FPS22
const unsigned int BYD_FPS_CONFIG_VERSION22 = CONFIG_FPS22;	//设置芯片版本号,见byd_fps_bf66xx.h
#endif

#ifdef BYD_FPS_SYSFS	
const unsigned short BYD_FPS_BUF_SIZE = BUF_TRANS_SIZE;
#endif
unsigned short byd_fps_fae_detect_para22[BYD_FPS_FAE_DETECT_LEN] = BYD_FPS_FAE_DETECT22;
unsigned char byd_fps_fae_detect22[12] = {0};//add os th
#if defined CONFIG_FPS12 || defined CONFIG_FPS11 || defined (CONFIG_FPS22)
unsigned short byd_fps_fae_detect_para[5] = BYD_FPS_FAE_DETECT12;
unsigned char byd_fps_fae_detect[10] = {0};//BYD_FPS_FAE_DETECT;
unsigned char byd_fps_fae_image[4] = BYD_FPS_FAE_IMAGE12;
#if defined(CONFIG_FPS12)||defined (CONFIG_FPS11)
unsigned short byd_fps_fae_fir[64] = BYD_FPS_FAE_FIR12;
#endif
#endif
#if defined(CONFIG_FPS22)

unsigned char byd_fps_fae_image22[3] = BYD_FPS_FAE_IMAGE22;
#endif

unsigned long set_timeout_finger_down=100;
unsigned long set_timeout_finger_up = 10;
unsigned long set_timeout_sub_scan = 20;
unsigned long set_timeout_image_scan = 100;

extern unsigned char byd_fps_chip_flag;
extern unsigned short byd_fps_chip_type;
...
{
	struct byd_fps_data *byd_fps = (struct byd_fps_data *)dev_id;
#endif
	int ret;
	ret =0 ;
	...
	if(byd_fps_int_test == 1){
		byd_fps_int_test = 0;
		printk("%s:byd int func\n",__func__);
	}
#ifdef BYD_FPS_INPUT_WAKEUP
	/*if ((byd_fps_instr_stop_flag != 1)&&(byd_fps_int_test!=1))*/
		ret = byd_fps_intr_work(); //最优先判断中断是否为异常中断suspend_flag:1,3--调用工作队列,在队列中处理: 采集、检测手指、比对指纹、发消息给上层.
		if(ret != 0) {
			goto exit;
		}
#endif
				...
				struct device_attribute *attr, char *buf)
{
	...
	switch (byd_last_cmd) {
		...
		if(byd_fps_chip_flag == 0x12){
			if(((byd_chip_info&0xf8c0)==BYD_FPS_CONFIG_VERSION12)||((byd_chip_info&0xf8c0)==0xa880)) {
				buf[0] = 0x40;
				buf[1] = 0x40;
			}
		}
		if(byd_fps_chip_flag == 0x11){
			if(((byd_chip_info&0xf8c0)==BYD_FPS_CONFIG_VERSION11)||((byd_chip_info&0xf8c0)==0xa880)) {
				buf[0] = 0x40;
				buf[1] = 0x40;
			}
		}
		if(byd_fps_chip_flag == 0x22){
			if(((byd_chip_info&0xf8c0)==BYD_FPS_CONFIG_VERSION22)||((byd_chip_info&0xf8c0)==0xa880)) {
				buf[0] = 0x40;
				buf[1] = 0x40;
			}
		...
		case 3:
			...
			buf[0] = buf[0]&0x04;
			#ifdef BYD_FPS_UP_DOWN_ASYNC
			if(( buf[0] != 0x04 )&&((g_need_finger_down_scan_flag = 1)&&(buf[1]!=1))){//???????,?????
				g_need_finger_down_scan_flag = 0;
				g_need_send_signal = 1;
				printk("%s:ESD int,need finger down send async again\n",__func__);
				byd_fps_set_sensor_finger_detected();
			}
			if(( buf[0] != 0x04 )&&((g_need_finger_up_scan_flag = 1)&&(buf[1]!=0))){
				g_need_finger_up_scan_flag = 0;
				byd_fps_finger_up_async_flag = 1;
				printk("%s:ESD int,need finger up send async again\n",__func__);
				byd_fps_set_sensor_finger_detected();
		...
		case 5:
			...
			byd_fps_keydown_flag = 1;//成功则开启按键
			#endif
		//1.单纯只是读取数据，不录入，不比对
		...
		case 19:
			...
			if (ReceiVeCmdType == 4) {  //读阈值 应该从寄存器里面取数 修改一下
				if((byd_fps_chip_flag == 0x11)||(byd_fps_chip_flag == 0x12)){
					...
					byd_fps_mutex_lock(2,1);
					printk("%s:cmd19,temp value is=%d\n", __func__,temp);
					throldvalue_on = 65535 - temp;
					printk("%s:cmd19,throldvalue_on is=%d\n", __func__,throldvalue_on);
					byd_fps_mutex_lock(1,1);
					temp = read_throld_off(byd_fps->spi);
					byd_fps_mutex_lock(2,1);
					printk("%s:cmd19,off temp value is=%d\n", __func__,temp);
					throldvalue_off = 65535 - temp;
					printk("%s:cmd19,throldvalue_off is=%d\n", __func__,throldvalue_off);
					buf[0] = throldvalue_on >> 8; //按下系数
					buf[1] = throldvalue_on & 0xff;	//抬起系数
					buf[2] = throldvalue_off >> 8;//变化量高位
					buf[3] = throldvalue_off & 0xff;
				}else if(byd_fps_chip_flag == 0x22){
					buf[0] = byd_fps_fae_detect_para22[0];//throldvalue_on >> 8; 按下系数
					buf[1] = byd_fps_fae_detect_para22[1];//throldvalue_on & 0xff;	抬起系数
					buf[2] = (otp_fpd_th*16) >>8; //throldvalue_off >> 8;变化量高位
					buf[3] = (otp_fpd_th*16) & 0xff;//throldvalue_off & 0xff;		
				}
				ret = 4;
				...
				if(byd_fps_chip_flag == 0x12){
					if((byd_chip_info&0xf8c0) == BYD_FPS_CONFIG_VERSION12){
						buf[0] = 18;
						buf[1] = BYD_FPS_SENSOR_HEIGHT12;
						buf[2] = BYD_FPS_SENSOR_WIDTH12;
				}
				}
				if(byd_fps_chip_flag == 0x11){
					if((byd_chip_info&0xf8c0) == BYD_FPS_CONFIG_VERSION11){
						buf[0] = 10;
						buf[1] = BYD_FPS_SENSOR_HEIGHT11;
						buf[2] = BYD_FPS_SENSOR_WIDTH11;
					}
				}
				if(byd_fps_chip_flag == 0x22){
					if((byd_chip_info&0xf8c0) == BYD_FPS_CONFIG_VERSION22){
						buf[0] = 22;
						if((byd_fps_chip_type == 0x5531) || (byd_fps_chip_type == 0x5532 ) ){
							buf[1] = BYD_FPS_SENSOR_HEIGHT_5531;
							buf[2] = BYD_FPS_SENSOR_WIDTH_5531;
						}else if(byd_fps_chip_type == 0x5541){
	...
	FP_DBG("byd byd_fps_bf66xx_spi_qsee_show() end, ret=%d\n", ret);
...
// *******************************************************************************
// * Function    :  byd_fps_read_image
// * Description :  capture fingerprint image
// * In          :  
// * Return      :  
// *******************************************************************************
// Input:  count == 0 读图像（同步阻塞式读）
// Return: 
//     0 -- 无手指或触摸不达标
//  size -- 读取到图像
//  -EINTR -- 被取消(等待手指阶段)
...
//  -ERESTARTSYS -- 异常
//----------------------------------------------------------------------
// Input:  count == 1 读图像（非阻塞）
// Return: 
//----------------------------------------------------------------------
// Input:  count == 2 读手指按下或抬起的状态寄（非阻塞）
// Return: 0 -- no touch or area not enough; 1 -- finger touched
//----------------------------------------------------------------------
// Input:  count == 3 读芯片当前工作模式（idle, 循环扫描，手指探测，休眠）
// Return: state = chip_mode
//--------------------------------------------------------------------- //
//#define BYD_DEBUG_IMAGE_DATA
int byd_fps_read_image(int count)
{
	int error;
	...
	unsigned int i=0;
	int ret;
	u8 count_temp;// count_cmd;

	struct byd_fps_data  *byd_fps = NULL;
#ifdef BYD_DEBUG_IMAGE_DATA
	u16 * ptrarray;
	int j,k;
	int temp;
#endif
	...
	count_temp = count&0x0F;
	if((count_temp != 2) && (count_temp != 3))
	{
		FP_DBG("\n%s: start capture : ...  \n", __func__);
		//change old interface to new interface.
		if(byd_fps_chip_flag == 0x22){
			if(count_temp == 0x0E) {	//OS SCAN need 积分次数为96次
				byd_fps_dig_inte_set(byd_fps, 1);
			}
		}
		byd_fps_instr_stop_flag = 1;
		error = byd_fps_task_capture(byd_fps, count_temp);
		byd_fps_instr_stop_flag = 0;
		if(byd_fps_chip_flag == 0x22){
			if(count_temp == 0x0E) {	//FP scan 积分次数为64次.
				byd_fps_dig_inte_set(byd_fps, 0);
			}
		}
		DBG_TIME("%s: capture END, return = %d!\n", __func__, error);
		if(error <= 0){
			if (error == -512) {	//-512(-ERESTARTSYS) --等待事件的过程被信号中断
				pr_warn("%s:ret=%d\n", __func__, error);
				msleep(50);
			}
			if(error == -10){
				pr_warn("%s:ret=%d,not proper fingerprint interrupt\n", __func__, error);
				error = 0;
				goto out;
			}
			if (error == -ETIMEDOUT) {
				pr_warn("%s: error == -ETIMEDOUT\n", __func__);
				error = 0;
				goto out;
			} else {
				pr_warn("%s: error == %d\n", __func__, error);
...
#ifdef BYD_DEBUG_IMAGE_DATA
		ptrarray = (unsigned short *)&byd_fps->data_buffer[1];
		/*for(j=0;j<160;j++)*/
		for(j=100;j<102;j++) {
			for(k=0;k<160;k++){
				DBG_TIME("0x%x,", ptrarray[j*160+k]);
				if((j*160+k)%20 == 0){
					DBG_TIME("\n");
				}
			}
...
#ifdef BYD_SAVE_IMAGE
		#define SAVE_TO_SD
		DBG_TIME("%s: byd write img\n", __func__);
		if(byd_fps_get_suspend_flag() == 0) {
			if(byd_fps_img_cnt < 100) {
				#ifdef SAVE_TO_SD
				byd_fps_img_file1[27] = 48+(byd_fps_img_cnt/10);
				byd_fps_img_file1[28] = 48+(byd_fps_img_cnt%10);
				#else
				byd_fps_img_file1[59] = 48+(byd_fps_img_cnt/10);
				byd_fps_img_file1[60] = 48+(byd_fps_img_cnt%10);
				#endif
				byd_fps_img_cnt++;
			} else {
				byd_fps_img_cnt = 0;
				#ifdef SAVE_TO_SD
				byd_fps_img_file1[27] = 48+0;
				byd_fps_img_file1[28] = 48+0;
				#else
				byd_fps_img_file1[59] = 48+0;
				byd_fps_img_file1[60] = 48+0;
				#endif
				byd_fps_img_cnt++;
			}/**/
			byd_fps_write_file(byd_fps_img_file1, &byd_fps->data_buffer[1], BYD_FPS_IMAGE_WIDTH*BYD_FPS_IMAGE_HEIGHT*2, 0);
		...
		if(count_temp == 2){
			error = byd_fps_auto_wait_finger_up_down_timeout(byd_fps, FINGER_UP, 0);

			if(ret == 0){ //手指符合要求
				error = FINGER_UP; //error = FINGER_UP;  //1 -- down, 0 -- up
			}else{
				error = FINGER_DOWN;
			}
		}
		if(count_temp == 3){
			error = byd_fps_chip_state(byd_fps->spi);
		}
	}

	out:
	work_cmd = 0; //sean 20141107
	DBG_TIME("%s: End,return =%d \n\n", __func__, error);
...
static ssize_t byd_fps_read(struct file *file, char *buf_u, size_t count, loff_t *ppos)
{
	...
	#ifdef BYD_FPS_TEST_TIME_LOG
	DBG_TIME("\nbyd fps read() IN, byd_last_cmd: %d,ReceiVeCmdType =%d,byd_fps_ageing_test_flag = %d\n", byd_last_cmd,ReceiVeCmdType,byd_fps_ageing_test_flag);
	#endif
	...
	switch (byd_last_cmd) {
		...
		case 3:
			...
			#ifdef BYD_FPS_UP_DOWN_ASYNC
			if(( buf_k[0] != 0x04 )&&((g_need_finger_down_scan_flag = 1)&&(buf_k[1]!=1))){//静电打出的中断，非正常中断
				g_need_finger_down_scan_flag = 0;
				g_need_send_signal = 1;
				printk("%s:ESD int,need finger down send async again\n",__func__);
				byd_fps_set_sensor_finger_detected();
			}
			if(( buf_k[0] != 0x04 )&&((g_need_finger_up_scan_flag = 1)&&(buf_k[1]!=0))){
				g_need_finger_up_scan_flag = 0;
				byd_fps_finger_up_async_flag = 1;
				printk("%s:ESD int,need finger up send async again\n",__func__);
				byd_fps_set_sensor_finger_detected();
		...
		case 19:
			...
			if(ReceiVeCmdType == 4){  //读阈值 应该从寄存器里面取数 修改一下
				...
				if((byd_fps_chip_flag == 0x11)||(byd_fps_chip_flag == 0x12)){
					...
					throldvalue_off = 65535 - temp;
					printk("%s:cmd19,throldvalue_off is=%d\n", __func__,throldvalue_off);
					buf_k[0] = throldvalue_on >> 8; //按下系数
					buf_k[1] = throldvalue_on & 0xff;	//抬起系数
					buf_k[2] = throldvalue_off >> 8;//变化量高位
					buf_k[3] = throldvalue_off & 0xff;
				}else if(byd_fps_chip_flag == 0x22){
					buf_k[0] = byd_fps_fae_detect_para22[0];//throldvalue_on >> 8; 按下系数
					buf_k[1] = byd_fps_fae_detect_para22[1];//throldvalue_on & 0xff;	抬起系数
					buf_k[2] = (otp_fpd_th*16) >>8; //throldvalue_off >> 8;变化量高位
					buf_k[3] = (otp_fpd_th*16) & 0xff;//throldvalue_off & 0xff;		
				}
				...
				if(byd_fps_chip_flag == 0x12){
					if((byd_chip_info&0xf8c0) == BYD_FPS_CONFIG_VERSION12){
						buf_k[0] = 18;
						buf_k[1] = BYD_FPS_SENSOR_HEIGHT12;
						buf_k[2] = BYD_FPS_SENSOR_WIDTH12;
					}
				}
				if(byd_fps_chip_flag == 0x11){
					if((byd_chip_info&0xf8c0) == BYD_FPS_CONFIG_VERSION11){
						buf_k[0] = 10;
						buf_k[1] = BYD_FPS_SENSOR_HEIGHT11;
						buf_k[2] = BYD_FPS_SENSOR_WIDTH11;
					}
				}
				if(byd_fps_chip_flag == 0x22){
					if((byd_chip_info&0xf8c0) == BYD_FPS_CONFIG_VERSION22){
...
// * Return      :  none
// *******************************************************************************
void byd_fps_remove(struct byd_fps_data *byd_fps)
{
	int ret;
	ret = 0;
	//退出算法
...
// *******************************************************************************
#ifdef BYD_FPS_ALG_IN_KERNEL
unsigned char CONFIG_ALG_NUM_TEMPLATES_PER_FENGER = 0;
unsigned char CONFIG_ALG_MAX_NUM_TEMPLATES = 0;
unsigned char CONFIG_ALG_SENSOR_MOVEXY = 0;
static int byd_fps_alg_para_init(void)
{
	...
	//-------------------------------CFG ALG PARA ACCORDING TO THE CHIP--------------------------
	if((byd_fps_chip_flag == 0x11)||(byd_fps_chip_flag == 0x12)){
		CONFIG_ALG_NUM_TEMPLATES_PER_FENGER = CONFIG_ALG_NUM_TEMPLATES_PER_FENGER_BIG;	
	}else if(byd_fps_chip_flag == 0x22){
		CONFIG_ALG_NUM_TEMPLATES_PER_FENGER = CONFIG_ALG_NUM_TEMPLATES_PER_FENGER_SMALL;
	}
	if(byd_fps_chip_flag == 0x11){
		CONFIG_ALG_MAX_NUM_TEMPLATES = CONFIG_ALG_MAX_NUM_TEMPLATES_11;
	}else if(byd_fps_chip_flag == 0x12){
		CONFIG_ALG_MAX_NUM_TEMPLATES = CONFIG_ALG_MAX_NUM_TEMPLATES_12;
	}else if(byd_fps_chip_flag == 0x22){
		CONFIG_ALG_MAX_NUM_TEMPLATES = CONFIG_ALG_MAX_NUM_TEMPLATES_22;
	}
	if(byd_fps_chip_flag == 0x12){
		CONFIG_ALG_SENSOR_MOVEXY = CONFIG_ALG_SENSOR_MOVEXY_BIG;
	}else if((byd_fps_chip_flag == 0x11)||(byd_fps_chip_flag == 0x22)){
		
		CONFIG_ALG_SENSOR_MOVEXY = CONFIG_ALG_SENSOR_MOVEXY_SMALL;
	}
	////////////////////////////////END CFG ALG PARA ACCORDING TO THE CHIP//////////////////
	//算法参数初始化值设置
...
//early suspend init
	...
	//ret = byd_fps_alg_version_init();	//must after "byd_fps_config_sensor_poweron()"
...
{
	long ret;
	FP_DBG("this_byd_fps->interrupt_done=%d befor\n", this_byd_fps->interrupt_done);
	this_byd_fps->interrupt_done = 0;
	FP_DBG("this_byd_fps->interrupt_done=%d present\n", this_byd_fps->interrupt_done);
	byd_fps_wait_flag = 1;
...
FILE *byd_fopen(char *path, char *type)
{
    ...
    if (IS_ERR(fp)) {
     // 2015-9-2 qiximing delete: pr_warn() call causes serious time delay (about 10ms) in kernel
        /*pr_warn("Warning: byd_fopen(): filp_open() FAILED: %s", path);*/
...
#endif
    if (buf == NULL)
        pr_err("byd_malloc(): kzalloc(size) or vmalloc(size) FAILED! size=%d \n", size);
    return buf;
}
