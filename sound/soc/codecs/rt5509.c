			    ...
			    const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		if (!pdata) {
			printk("rt5509 Failed to allocate memory\n");
			return -ENOMEM;
		}
		...
		if (!pdata) {
			printk("rt5509 Failed, no pdata specified\n");
			return -EINVAL;
		}
	}
	...
	if (ret < 0) {
		printk("rt5509 power on fail 1\n");
		goto err_sw_reset;
	}
	/* do software reset at default */
	ret = rt5509_sw_reset(chip);
	if (ret < 0) {
		printk("rt5509 sw_reset fail\n");
		goto err_sw_reset;
	}
	ret = _rt5509_power_on(chip, true);
	if (ret < 0) {
		printk("rt5509 power on fail 2\n");
		goto err_pm_init;
	}
	/* get chip revisioin first */
	ret = rt5509_get_chip_rev(chip);
	if (ret < 0) {
		printk("rt5509 get chip rev fail\n");
		goto err_sw_reset;
	}
	/* register RegMAP */
	...
	if (!chip->rd) {
		printk("rt5509 create regmap device fail\n");
		ret = -EINVAL;
		goto err_regmap;
	}
	ret = rt5509_i2c_initreg(chip);
	if (ret < 0) {
		printk("rt5509 init_reg fail\n");
		goto err_initreg;
	}
	ret = rt5509_handle_pdata(chip);
	if (ret < 0) {
		printk("rt5509 init_pdata fail\n");
		goto err_pdata;
	}
	ret = rt5509_power_on(chip, false);
	if (ret < 0) {
		printk("rt5509 power off fail\n");
		goto err_put_sync;
	}
	...
	if (ret < 0) {
		dev_err(chip->dev, "rt5509 codec register fail\n");
		goto err_put_sync;
	}
	dev_cnt++;
	printk("rt5509 successfully driver probed\n");
	return 0;
err_put_sync:
err_pdata:
err_initreg:
